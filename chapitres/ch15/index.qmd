...
<!-- comment -->
Certaines fonctions `f` ont une expression explicite de leurs racines.
<!-- comment -->
Par exemple :

<!-- paragraph -->

- la fonction linéaire `f(x)=b*x+c=0` a une seule racine `x=-c/b` lorsque b est non nul.
<!-- comment -->
- la fonction quadratique `f(x)=a*x^2+b*x+c=0` a deux racines `x=(-b±sqrt(b^2-4*a*c))/(2*a)` si le discriminant `b^2-4*a*c>0` est positif.
<!-- comment -->
- la fonction sin `f(x)=sin(a*x)=0` a un nombre infini de racines : `pi*z/a` pour tous les entiers `z`.

<!-- paragraph -->

Cependant, certaines fonctions n’ont pas d’expression explicite de leurs racines.
<!-- comment -->
...
<!-- comment -->
(en fait, une solution faisant appel à la [fonction W de Lambert](https://www.wolframalpha.com/input/?i=a*x+%2Bb*log%28x%29%2B+c%3D0) existe mais cette fonction n’est généralement pas disponible). L’objectif de ce chapitre est de créer une visualisation des données interactive qui explique [la méthode de Newton](https://fr.wikipedia.org/wiki/M%C3%A9thode_de_Newton) pour trouver les racines de telles fonctions.
...
<!-- comment -->
Nous créons plusieurs visualisations statiques et une visualisation interactive.
<!-- comment -->
...
<!-- comment -->
La méthode de Newton pour la recherche de racines utilise comme point de départ une racine candidate choisie arbitrairement, puis applique de manière itérative des approximations linéaires pour trouver des racines candidates plus précises.
<!-- comment -->
...
Le graphique ci-dessus montre la racine trouvée.
<!-- comment -->
Le critère d’arrêt était une valeur de coût absolu inférieure à `1e-6`; nous savons donc que cette racine est au moins aussi précise que ce seuil.
<!-- comment -->
...
<!-- comment -->
Les panneaux de droite montrent la racine de l’approximation linéaire, en plus de la valeur de la fonction correspondante (la nouvelle racine candidate).
...
On constate aisément que l’algorithme converge rapidement vers la racine.
<!-- comment -->
...
Le code ci-dessous utilise la fonction W de Lambert pour calculer une racine et compare la solution trouvée à celle obtenue par la méthode de Newton.
...
<!-- comment -->
Dans celle-ci, nous explorons un algorithme permettant de trouver l’autre racine (plus petite que le minimum).
<!-- comment -->
...
<!-- comment -->
Créez un `animint2` similaire à celle de la section précédente.
...
Dans ce chapitre, nous avons exploré plusieurs visualisations de la méthode de Newton pour trouver les racines de fonctions continues.
...
<!-- comment -->
- Ajoutez une légende de taille au premier graphique (des points noirs  plus grands que les rouges), afin qu’on puisse distinguer les cas où les points ont la même valeur.
<!-- comment -->
- Ajoutez un `geom_hline()` pour mettre en évidence la valeur de perte=0 dans le second graphique.
<!-- comment -->
- Ajoutez un `geom_hline()` pour mettre en évidence le seuil d’arrêt dans le premier graphique.
<!-- comment -->
- Désactivez l’une des deux légendes pour gagner de l’espace.
<!-- comment -->
- Comment spécifier des transitions graduelles entre les itérations ?
<!-- comment -->
- Au lieu d’utiliser l’itération comme variable d’animation/de temps, créez-en une nouvelle afin de montrer deux états/étapes distincts pour chaque itération, c’est-à-dire la variable `step` dans le graphique à facettes ci-dessus.
<!-- comment -->
...
<!-- comment -->
Réalisez une visualisation des données qui permet la sélection de la valeur de départ et qui montre le nombre d’itérations nécessaires pour converger, en respectant le seuil.
<!-- comment -->
- Créez un autre graphique permettant de sélectionner le seuil.
<!-- comment -->
Tracez le nombre d’itérations en fonction du seuil.
<!-- comment -->
- Dérivez la fonction de perte pour la [régression binomiale](https://en.wikipedia.org/wiki/Binomial_regression) (en anglais seulement) et visualisez la méthode de recherche de racines de Newton correspondante.
<!-- comment -->
...
Dans le [chapitre 16](/ch16), nous vous expliquerons comment visualiser les modèles de détection des points de rupture.
