...
La plupart des systèmes d’analyse de données offrent des outils de création de graphiques pour visualiser les tendances des données.
<!-- comment -->
...
<!-- comment -->
Lorsqu’il recourt à l’un de ces systèmes généraux, l’utilisateur doit assembler lui-même les composantes pour obtenir un graphique significatif et interprétable.
<!-- comment -->
L’avantage de ces systèmes réside dans le peu de limites imposées aux types de graphiques qui peuvent être créés.
<!-- comment -->
Leur principal inconvénient est qu’ils ne fournissent généralement pas de fonctions pour automatiser les tâches courantes (axes, panneaux, légendes).

<!-- comment -->

Pour remédier aux inconvénients de ces systèmes, des packages de visualisation tels que `lattice` ont été développés ([Sarkar, 2008](http://lmdvr.r-forge.r-project.org/figures/figures.html)).
<!-- comment -->
Ils disposent de plusieurs types de graphiques prédéfinis et fournissent une fonction dédiée à la création de chacun d’eux.
<!-- comment -->
Par exemple, `lattice` offre la fonction `bwplot` pour concevoir des diagrammes en boîte et des boîtes à moustaches.
<!-- comment -->
L’avantage de ces systèmes est qu’ils facilitent grandement la création de graphiques complets avec légendes et panneaux.
<!-- comment -->
Par contre, ils proposent un ensemble de modèles de graphiques prédéfinis, ce qui complique la création de graphiques plus complexes.
<!-- comment -->

Les nouveaux systèmes basés sur la grammaire des graphiques se situent entre ces deux extrêmes.
<!-- comment -->
Leland Wilkinson a proposé la grammaire des graphiques pour décrire et créer un grand éventail de graphiques ([Wilkinson, 2005](http://www.springer.com/us/book/9780387245447?wt_mc=GoogleBooks.GoogleBooks.3.EN&token=gbgen#otherversion=9780387286952)).
<!-- comment -->
...
Dans ce manuel, nous utilisons le mot « esquisse » dans le sens de « Première forme, traitée à grands traits et généralement en dimensions réduites, de l’œuvre projetée » ([Larousse](https://www.larousse.fr/dictionnaires/francais/esquisse/31075)).
Nous vous présenterons souvent l’esquisse dessinée à la main du graphique à créer, avant d’écrire le code nécessaire.
Chaque esquisse comportera certains éléments qui nous permettront d’écrire facilement le code correspondant.
Nous vous encourageons à faire vos propres esquisses dans un cahier ou sur un tableau avant d’écrire le code du graphique que vous voulez créer.
<!-- comment -->
Pour notre exemple, nous utilisons un ensemble de données de la Banque mondiale.
Commençons par charger et examiner le nom des colonnes.
...
<!-- comment -->
Le jeu de données `BanqueMondiale` comprend des mesures telles que le taux de fertilité et l’espérance de vie pour chaque pays pour la période 1960-2010.
<!-- comment -->
Pour simplifier les sorties ci-dessous, nous considérons seulement le sous-ensemble pertinent pour les visualisations :
...
![Esquisse d’un nuage de points pour les données de la banque mondiale](ch02-viz-scatter.png)
...
<!-- comment -->
Le jeu de données `banque_mondiale` possède déjà ces colonnes, il suffit donc de considérer le sous-ensemble pour 1975 :
...
<!-- comment -->
L’étape suivante consiste à utiliser les notes de l’esquisse pour coder un ggplot avec `aes()` qui établit la correspondance entre variables et propriétés visuelles :

<!-- comment -->

```{r ch02-vis-scatter-ggplot}
...
<!-- comment -->
Le ggplot a été sauvegardé dans une variable nommée `nuage_de_points` qui, lorsqu’imprimée sur la ligne de commande R, affiche le graphique sur un périphérique graphique.
<!-- comment -->
Notez que nous avons automatiquement une légende `région` en couleur.

## Afficher des ggplots sur des pages web à l’aide de animint {#rendering-with-animint}
...
```{r ch02-vis-scatter-animint}
animint(nuage_de_points)
```

Lorsqu’on le visualise dans un navigateur web, le graphique ressemble aux versions statiques produites par les périphériques graphiques R standard,
<!-- comment -->
sauf pour la légende, qui est interactive : en cliquant sur celle-ci, les points de cette couleur sont cachés ou affichés.
<!-- comment -->
À l’interne, la fonction `animint()` crée une liste de classe `animint`, puis R exécute la fonction `print.animint()` grâce au [système d'objets S3](http://adv-r.had.co.nz/OO-essentials.html#s3).
La fonction `animint2dir()` est appelée pour compiler la liste en un dossier de fichiers de données (CSV) et de codes (HTML, JavaScript) affichables dans un navigateur web.
<!-- comment -->
Dans RStudio, vous devriez voir le graphique s’afficher sous l’onglet « visualiseur ».
Vous pouvez configurer le navigateur d’affichage avec l’option `browser`, par exemple : `options(browser="firefox")`.
<!-- comment -->
Lors de l’exécution du code ci-dessus, si l’animint ne s’affiche pas dans votre navigateur web pour une raison quelconque (par exemple, si vous voyez une page web vierge), veuillez consulter notre [FAQ](https://github.com/tdhock/animint2/wiki/FAQ#web-browser-on-local-indexhtml-file-is-blank). 

<!-- comment -->

**Exercices** : modification du contenu du `aes()` du ggplot, puis création d’une nouvelle visualisation.

* Utilisez la variable qualitative `revenu` dans `aes(color)` ou `aes(fill)`pour obtenir une légende interactive.
* Essayez `population` et `log10(population)` sur `x` ou `y`, vous constaterez que la transformation logarithmique fonctionne bien pour cette variable (`population` varie de plusieurs ordres de grandeur).
* Essayez `aes(size=log10(population))` ; vous obtiendrez une légende quantitative, qui ne sera pas interactive.
* Pour avoir une légende interactive pour `population`, il faudra discrétiser la variable. Créez une nouvelle variable discrète `log10pop` avec définition `paste(round(log10(population)))`. Si vous utilisez `aes(size=log10pop)` vous obtiendrez une légende qualitative interactive.

## Graphiques multicouches (plusieurs geoms) {#multi-layer}

<!-- comment -->

Dans les ggplots, nous utilisons le mot « couche » comme synonyme de « geom », par exemple `geom_point()` que nous avons utilisé ci-dessus.
D’autres geoms existent, tels que `geom_line()` et `geom_path()` qui s’utilisent pour dessiner des courbes.
On appelle un graphique « multicouche » lorsqu’on utilise plusieurs geoms dans le même graphique, dessinés les uns sur les autres.
Cette approche s’avère utile pour afficher plusieurs geoms ou ensembles de données dans le même graphique.
Considérons par exemple l’esquisse suivante qui ajoute une couche de `geom_path()` à la visualisation de données précédente :

<!-- comment -->

![Esquisse d’un graphique à multiples couches pour les données de la banque mondiale](ch02-two-layer-viz.png)

<!-- comment -->

Il faut remarquer que l’esquisse ci-dessus comprend deux geoms différents (point et path).
<!-- comment -->
Les deux geoms partagent une définition commune des éléments `x`, `y`, et `color`, mais s’appliquent à des ensembles de données différents.
De plus, le `geom_path()` contient `group=pays`, ce qui signifie qu’il faut dessiner une courbe pour chaque valeur de `pays`.
...
Dans le code ci-dessus, nous créons d’abord un nouveau jeu de données `banque_mondiale_avant_1975`, puis nous ajoutons un `geom_path()` au `nuage_de_points` ce qui donnne `gg_deux_couches` un ggplot à deux couches.
<!-- comment -->
Nous sauvegardons ensuite la valeur rendue par l’appel à `animint()` dans l’objet `vis.deux.couches`, qu’on imprime pour l’afficher.
<!-- comment -->
...
L’ajout de `geom_path()` montre l’évolution des pays au fil du temps.
<!-- comment -->
Pour la plupart des pays, les données se sont déplacées vers la droite et vers le bas, ce qui indique une espérance de vie plus élevée et un taux de fertilité plus faible.
<!-- comment -->
...
Par ailleurs, certains pays ont enregistré une hausse de leur taux de fertilité.
...
<!-- comment -->
Vous pouvez utiliser `scale_color_manual` avec une palette de couleurs séquentielle. Pour plus de détails, consulter `RColorBrewer::display.brewer.all(type="seq")` et lire [l’annexe](/ch99#manual-color-legends).

<!-- comment -->

Pouvons-nous ajouter le nom des pays à la visualisation de données ?
<!-- comment -->
Ci-dessous, nous ajoutons une autre couche avec une étiquette de texte pour chaque pays :

<!-- comment -->

```{r ch02-vis-three-layer-vis}
...
<!-- comment -->
Cependant, il serait préférable que l’utilisateur puisse afficher et masquer individuellement le texte pour chaque pays.
<!-- comment -->
Ce type d’interaction s’obtient grâce aux mots-clés `showSelected` et `clickSelects` que nous expliquons dans les chapitres 3 et 4.
...
![Esquisse avec deux graphiques pour les données de la banque mondiale](ch02-viz-two-plots.png)

<!-- comment -->

Notez que l’esquisse ci-dessus peut être directement traduite dans le code R ci-dessous.
<!-- comment -->
Nous copions ensuite la visualisation existante (`vis.deux.couches`), puis nous ajoutons un ggplot dans un nouvel élément nommé `timeSeries` :
...
<!-- comment -->
Puisque la liste contient deux ggplots, `animint2` affiche la visualisation sous la forme de deux graphiques liés :

<!-- comment -->

```{r ch02-vis-two-plots}
vis.deux.graphes
```

<!-- comment -->

La visualisation ci-dessus contient deux ggplots, qui mappent chacun différentes variables de données sur le plan horizontal `x`.
<!-- comment -->
...
<!-- comment -->
Étant donné que `aes(color=région)` a été défini pour les deux graphiques, `animint` crée une seule variable de sélection partagée appelée `région`.
<!-- comment -->
Cliquer sur l’une des légendes a pour effet de mettre à jour l’ensemble des régions sélectionnées, `animint` actualise donc les légendes et les données dans les deux graphiques en conséquence.
<!-- comment -->
...
<!-- comment -->
Par exemple, vous pouvez ajouter une série temporelle d’une autre variable de données, comme `espérance.de.vie` ou `population`.
...
Un ggplot peut comporter des facettes (panneaux), c’est-à-dire des sous-graphiques présentant un sous-ensemble de données différent.
Le terme « facette » fait référence aux fonctions `facet_*` correspondantes.
<!-- comment -->
L’un des principaux atouts des ggplots est qu’il est relativement facile de créer différents types de graphiques avec plusieurs facettes.
<!-- comment -->
Les facettes répondent à deux objectifs :

<!-- comment -->

* L’alignement des axes de plusieurs graphiques connexes contenant différents geoms, ce qui facilite la comparaison entre les geoms et s’avère utile pour la visualisation interactive des données.
<!-- comment -->
* La division des données d’un geom en plusieurs panneaux, ce qui facilite la comparaison entre des sous-ensembles de données. Cette approche est moins utile pour la visualisation interactive (l’interactivité peut souvent remplacer les facettes et produire le même effet comparatif pour les sous-ensembles de données).

<!-- comment -->

### Geoms distincts dans chaque panneau (axes alignés) {#aligned-axes}

<!-- comment -->

Les facettes sont utiles pour aligner les axes quand on veut afficher différents geoms dans chaque facette.
<!-- comment -->
Considérons l’esquisse ci-dessous qui contient un graphique avec deux panneaux :

![Esquisse avec facettes alignées pour les données de la banque mondiale](ch02-viz-aligned.png)

<!-- comment -->

Notez que les deux panneaux tracent des geoms différents, chacun avec son propre `aes()`.
<!-- comment -->
Le `geom_point()` et le `geom_path()` dans le panneau de gauche ont `x=espérance.de.vie`, tandis que le `geom_line()` du panneau de droite utilise `x=année`.
<!-- comment -->
...
<!-- comment -->
Le panneau de gauche affiche les `geom_point()` et `geom_path()` et le panneau de droite affiche le `geom_line()`.
<!-- comment -->
Les panneaux ont un axe commun pour le taux de fertilité, ce qui facilite la comparaison directe entre le `geom_line()` du panneau des séries temporelles et le `geom_point()` et le `geom_path()` du panneau du nuage de points.

<!-- comment -->

La méthode employée ici utilise `add.x.var` pour ajouter une variable `x.var` à chaque ensemble de données, puis nous avons utilisé cette variable dans la fonction `facet_grid(scales="free")`.
<!-- comment -->
Nous appelons cette méthode [Ajouter une variable ensuite des facettes](/ch99#addColumn-then-facet). Elle est particulièrement utile pour créer une visualisation de données à plusieurs panneaux avec des axes alignés.
<!-- comment -->
Ainsi, pour changer l’ordre des panneaux dans la visualisation, il suffit de modifier l’ordre des niveaux de facteurs dans la définition de `add.x.var`.

<!-- comment -->

Notez également que `theme_bw()` produit des bordures de panneaux noires et des fonds de panneaux blancs, et que `theme(panel.margin=0)` pourrait être utilisé pour supprimer l’espacement entre les panneaux.
<!-- comment -->
Cette élimination libère de l’espace pour les panneaux, ce qui met l’accent sur les données.
<!-- comment -->
Nous appelons cette méthode [Les facettes économes en espace](/ch99#space-saving-facets), elle est généralement utile dans tout ggplot avec des facettes.
...
<!-- comment -->
* en augmentant la largeur du graphique à l’aide de `theme_animint()` voir le [chapitre 6](/ch06#plot-height-width) pour plus d’informations.
...
<!-- comment -->
* On souhaite comparer différents sous-ensembles de données tracées pour un geom.
...
![Esquisse avec facettes pour trois ans des données de la banque mondiale](ch02-viz-panels.png)
...
<!-- comment -->
Le `geom_point()` ne contient des données que pour 3 ans ; cependant le `geom_path()` contient des données pour 15 ans (mais 3 valeurs de `afficher.année`).
<!-- comment -->
Le code ci-dessous crée ces deux ensembles de données pour 3 années de l’ensemble de données de la Banque mondiale :
...
<!-- comment -->
Il s’agit d’un exemple de la méthode [Liste de tableau de données](/ch99#list-of-data-tables), utile pour la visualisation interactive :
...
La visualisation ci-dessus contient un seul ggplot avec trois facettes.
<!-- comment -->
Elle montre une portion plus étendue de l’ensemble des données de la Banque mondiale que les visualisations précédentes qui se limitaient aux données de 1975.
<!-- comment -->
Cependant, elle ne montre qu’un sous-ensemble de données relativement restreint.
<!-- comment -->
Vous pourriez être tenté d’utiliser un panneau pour afficher chaque année (plutôt que seulement 1975, 1985 et 1995).
<!-- comment -->
Sachez toutefois que ce type de visualisation à plusieurs panneaux fonctionne bien seulement s’il y a un nombre limité de sous-ensembles de données.
<!-- comment -->
...
Plutôt que de montrer toutes les données à la fois, nous pouvons créer une visualisation de données animée qui présente à l’observateur différents sous-ensembles de données au fil du temps.
<!-- comment -->
Dans le [chapitre suivant](/ch03) nous verrons comment le nouveau mot-clé `showSelected` peut être utilisé pour réaliser une animation, et nous révélerons plus de détails sur cet ensemble de données.
...
<!-- comment -->
* Nommez les deux raisons principales menant à la création de graphiques à plusieurs facettes ? Laquelle s’applique plus particulièrement aux visualisations interactives ?
<!-- comment -->
* Si on dit que dans « A < B » un élément B peut contenir plusieurs A, laquelle des affirmations suivantes est vraie ?
  * ggplot < facette
  * facette < ggplot
  * ggplot < animint
  * animint < ggplot
...
<!-- comment -->
* Dans `vis.alignée` le nuage de points affiche le taux de fertilité et l’espérance de vie, mais la série chronologique n’affiche que le taux de fertilité. Créez une variante de la visualisation qui affiche les deux séries temporelles. Indice : utilisez les panneaux horizontaux et verticaux dans `facet_grid`.
<!-- comment -->
...
<!-- comment -->
* Créez une visualisation à plusieurs panneaux qui montre chaque année des données `banque_mondiale` dans un panneau distinct. Quelles sont les limites de l’utilisation de graphiques statiques pour visualiser ces données ?
<!-- comment -->
* Créez un `vis.alignée` en utilisant un système de tracé qui n’est pas basé sur la grammaire des graphiques. Par exemple, vous pouvez utiliser les fonctions du package `graphics` dans R (`plot`,`points`,`lines` etc.), ou `matplotlib` en Python. Quels sont les avantages de `ggplot2` et de `animint` ?

<!-- comment -->

Dans le [chapitre 3](/ch03) nous expliquons le mot-clé `showSelected`, qui nous permettra de préciser l’interactivité ainsi que l’animation.
