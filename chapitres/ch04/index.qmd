...
<!-- comment -->
Le mot-clé `clickSelects` précise quelle variable de sélection sera mise à jour lorsqu’on clique sur le geom correspondant.
<!-- comment -->
...
<!-- comment -->
En fait, lorsque nous spécifions `color=région`  pour le `geom_point()`, `animint2` fait deux choses automatiquement :
...
* `duration=list(année=2000)` définit un délai de 2 secondes pour les transitions fluides entre les années.
* Nous avons vu dans le [chapitre 3](/ch03#duration-key) qu’il est important d’utiliser `aes(key)` pour chaque geom avec transition fluide.
...
<!-- comment -->
Vous devriez également constater que le point correspondant au pays sélectionné est plus foncé que les autres.
<!-- comment -->
...
<!-- comment -->
En cliquant sur « Next », vous obtiendrez des informations sur le prochain geom. En cliquant sur « Done » ou sur l’arrière-plan gris, vous mettrez fin à la visite guidée.
<!-- comment -->
La visite guidée aidera les nouveaux utilisateurs à découvrir les fonctions interactives présentes dans chaque geom.
<!-- comment -->
Les informations affichées pendant la visite sont personnalisables, il suffit de définir les paramètres `help` et `title` de chaque geom (voir le [chapitre 6](/ch06#custom-guided-tour) pour un exemple).
...
Essayez de cliquer sur l’entrée de la légende qui correspond à la région du pays sélectionné (par exemple, si le Canada est sélectionné, cliquez sur l’entrée de la légende Amérique du Nord).
<!-- comment -->
...
Dans le chapitre précédent, nous avons introduit les termes « manipulation directe » et « manipulation indirecte » pour décrire les interactions avec les légendes et les menus.
<!-- comment -->
...
<!-- comment -->
L’esquisse ci-dessous ne présente qu’une seule différence par rapport à celle de la dernière section : les étiquettes de texte sont affichées pour plus d’un pays.
...
Il est facile d’ajouter des pays à la sélection en cliquant sur des points de données.
<!-- comment -->
...
Exercice  : animez la visualisation de données en spécifiant l’option `time`, comme expliqué au [chapitre 3](/ch03#animation-time).

<!-- paragraph -->

Exercices avec multicouches : ajoutez un `geom_text()` qui indique l’année en cours sur le nuage de points.
<!-- comment -->
Ajoutez un `geom_path()` qui présente les données des cinq dernières années.
...
Tout d’abord, nous recréons le nuage de points de la section précédente à l’aide de la méthode [ajouter une variable ensuite des facettes](/ch99#addColumn-then-facet), utile pour créer des ggplots avec des axes alignés.
...
<!-- comment -->
Pour plus d’informations, voir la discussion et l’exemple dans la section [préciser le mode d’affichage de l’état de la sélection, au chapitre 6](/ch06#display-selection-state).
...
<!-- comment -->
Puisque nous avons spécifié `clickSelects="année"` pour le `geom_tallrect()`, en cliquant sur un rectangle on met à jour l’année sélectionnée.
...
<!-- comment -->
- Quand `alpha=0.75` est utilisé pour `geom_point()` avec les sorties graphiques habituelles dans R, quel est le degré d’opacité pour tous les points de données ?
<!-- comment -->
Lorsque `animint2` affiche le même geom, certains points seront sélectionnés et d’autres non.
<!-- comment -->
Quelle est l’opacité des points sélectionnés ?
<!-- comment -->
Quelle est l’opacité des points non sélectionnés ?
<!-- comment -->
...
Dans le [chapitre 5](/ch05), nous expliquons différentes méthodes pour publier et partager des animints sur le web.
