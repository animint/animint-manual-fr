...
Ci-dessus on contate la présence d’une nouvelle colonne `auc.task.rank` qui présente des valeurs entre 1 et 4.
<!-- comment -->
...
Pour tester les différences entre les rangs adjacents, nous effectuaons une jointure entre chaque rang (`auc.task.rank`) et le suivant (`next.rank`).
...
Exercice : combinez sur un même grapfique les valeurs-p avec le nom des algorithmes et les statistiques (moyenne et écart type). Utilisez ce graphique au lieu de `testAUCzoom` dans la visualisation précédente.
...
<!-- comment -->
* Pour le modèle lineaire avec régularisation L1 (`glmnet`), il faut augmenter la pénalité pour régulariser. Une petite pénalité peut donner un surapprentissage; une grande penalité, un sous-apprentissage.
