...
* `spam` représente une classification de courriels (`spam` étant un pourriel et `nonspam` un courriel légitime) à partir d’un vecteur de comptage de mots « bag of words » [@spambase].
<!-- comment -->
* `sonar` représente une classification entre métal et roche à partir d’un vecteur de bandes de fréquences sonores [@gorman1988analysis].
<!-- comment -->
* `vowel` représente une classification du son des voyelles, à partir de dix variables extraites d’un fichier audio [@vowel].
<!-- comment -->
* `waveform` est un jeu de données de simulation [@Hastie2009].
<!-- comment -->
* `zip` représente une classification de chiffres manuscrits, à partir d’une image de 16x16 pixels, en échelle de gris [@lecun1990].
...
On ne voit plus les résultats pour `featureless` dans le graphique ci-dessus.
<!-- comment -->
