...
Le graphique ci-dessous combine les poids du lasso avec le graphique des erreurs de prédiction pour les ensembles entraînement et test.
...
<!-- comment -->
Tout d’abord, vous avez peut-être remarqué qu’il y a deux légendes de `set` différentes dans la visualisation précédente (`linetype=set` dans le premier graphique de chemin et `color=set` dans le second graphique de résidus).
<!-- comment -->
...
Deuxièmement, nous avons remplacé le `geom_tallrect()` unique du premier graphique par deux.
<!-- comment -->
Le premier `geom_tallrect()` a `showSelected="arclength"` et est utilisé pour afficher la norme L1 (`arclength`) sélectionnée, à l’aide d’un rectangle gris.
<!-- comment -->
...
<!-- comment -->
Le deuxième `geom_tallrect()` a `clickSelects="arclength"` et le fait de cliquer dessus a pour effet de modifier la valeur sélectionnée de `arclength`.
<!-- comment -->
