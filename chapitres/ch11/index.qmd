...
L’objectif de ce chapitre est de créer une visualisation des données interactive qui explique le [Lasso](https://fr.wikipedia.org/wiki/Lasso_(statistiques)), un modèle d'apprentissage automatique pour la régression linéaire régularisée.
...
## Graphiques statiques de la régularisation {#static-path-plots}
...
<!-- comment -->
Le chemin commence à la solution des moindres carrés (`lambda=0`) à gauche, et se termine par le modèle à ordonnée à l’origine complètement régularisé à droite.
<!-- comment -->
<!-- comment -->
Pour voir la correspondance avec la solution des moindres carrés ordinaires, nous ajoutons des points dans le graphique ci-dessous.
...
Dans le prochain graphique, nous montrons le chemin en fonction de la norme L1 (arclength), avec quelques points supplémentaires sur une grille régulièrement espacée que nous utiliserons plus tard pour l’animation.
...
<!-- comment -->
L’algorithme LARS fournit rapidement des solutions Lasso pour autant de points de grille que souhaité.
<!-- comment -->
...
## Visualisation interactive de la régularisation {#interactive-path-vis}
...
Ci-dessous, nous combinons les ggplots présentés plus haut en un seul `animint2`.
<!-- comment -->
En cliquant sur le premier graphique, on modifie le paramètre de régularisation et les résidus affichés dans le second graphique.

<!-- paragraph -->

```{r ch11-vis-one-split}
animint(
  gg.path,
  gg.res,
...
## Refonte avec des rectangles mobiles {#re-design}

<!-- paragraph -->

La refonte ci-dessous comporte deux changements.
<!-- comment -->
Tout d’abord, vous avez peut-être remarqué que la visualisation précédente présente deux légendes distinctes pour la variable set (`linetype=set` dans le premier graphique de chemin et `color=set` dans le second graphique de résidus).
<!-- comment -->
Le décodage serait plus facile pour le lecteur si la variable `set` n’était mappée qu’une seule fois.
<!-- comment -->
Ainsi, dans la visualisation ci-dessous, nous remplaçons le `geom_point()` dans le deuxième graphique par un `geom_segment()` avec `linetype=set`.

<!-- paragraph -->

Le second changement est que nous avons remplacé le `geom_tallrect()` unique du premier graphique par deux autres.
<!-- comment -->
Le premier `geom_tallrect()` contient `showSelected="arclength"` et sert à afficher la norme L1 (`arclength`) sélectionnée, à l’aide d’un rectangle gris.
<!-- comment -->
Puisque nous spécifions une durée `duration` pour la variable `arclength` et `key=1`, nous observerons une transition graduelle du rectangle gris sélectionné.
<!-- comment -->
Le deuxième `geom_tallrect()` contient `clickSelects="arclength"` de sorte que cliquer dessus modifie la valeur sélectionnée de `arclength`.
<!-- comment -->
Nous spécifions un autre ensemble de données avec plus de lignes, et utilisons les [variables `clickSelects` et `showSelected` nommées](/ch06#data-driven-selectors) pour indiquer que `arclength` doit également être utilisé comme une variable `showSelected`.

<!-- paragraph -->

```{r ch11-vis-moving-rect}
...
Nous avons créé une visualisation du modèle d’apprentissage automatique Lasso, qui montre simultanément le chemin de régularisation et les courbes d’erreur.
<!-- comment -->
...
<!-- comment -->
- À quoi ressembleraient les courbes d’erreur si d’autres répartitions entraînement/validation étaient choisies ?
<!-- comment -->
Effectuez une validation croisée à quatre plis et ajoutez un graphique permettant de sélectionner le pli de test.

<!-- paragraph -->

Dans le [chapitre 12](/ch12), nous vous expliquerons comment visualiser la machine à vecteurs de support.
