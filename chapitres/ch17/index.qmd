...
<!-- comment -->
- Nous choisissons de manière aléatoire K= 3 points de données à utiliser comme centres d’agrégation.
<!-- comment -->
...
## Visualisation des données Iris avec des étiquettes {#vis-iris}
...
Ci-dessus, nous avons affiché les deux ensembles de données (centres d’agrégation et données) à l’aide de deux `geom_point()`.
<!-- comment -->
...
Il y a `r nrow(seg.dt)` segments superposés ci-dessus, si bien que l’interactivité serait utile pour mettre en évidence les segments connectés à un point de données précis.
<!-- comment -->
À cet effet, nous créons ci-dessous une variable de sélection `data.i` :
...
Les visualisations ci-dessus illustrent les étapes de la méthode K-moyennes : (1) mise à jour de l’agrégation sur la base du centre le plus proche (2) mise à jour du centre à partir des données affectées à cette agrégation.
<!-- comment -->
Pour visualiser plusieurs itérations des deux étapes ci-dessus, nous utilisons une boucle for.
...
<!-- comment -->
- Le code actuel impose un nombre maximal d’itérations, il est donc possible que les dernières ne progressent pas.
<!-- comment -->
Par exemple, l’itération 16 est la dernière à réduire l’erreur (les itérations 17 à 20 n’entraînent aucune diminution).
<!-- comment -->
Modifiez le code pour arrêter l’itération en absence de diminution de l’erreur.
<!-- comment -->
- La visualisation actuelle n’a qu’un cadre d’animation (sous-ensemble `showSelected`) par itération (la moyenne est affichée avant sa mise à jour).
<!-- comment -->
...
<!-- comment -->
Calculez l'indice de Rand ajusté en utilisant `pdfCluster::adj.rand.index(species, cluster)` pour chaque K et graine aléatoire.
<!-- comment -->
...
Dans le [chapitre 18](/ch18), nous vous expliquerons comment visualiser l’algorithme d’apprentissage par descente de gradient pour l’apprentissage des réseaux neuronaux.
