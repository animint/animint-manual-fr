...
<!-- comment -->
Ces données sont fournies par de médecins de l’Institut Curie (Paris, France) qui ont annoté visuellement les régions avec et sans points de rupture.
<!-- comment -->
...
En examinant le graphique des erreurs d’étiquettes ci-dessus, il devient évident que le modèle à quatre segments devrait être sélectionné, car il permet d’obtenir un nombre nul d’erreur d’étiquette.
<!-- comment -->
Un certain nombre de méthodes peuvent être utilisés pour sélectionner le meilleur modèle.
<!-- comment -->
...
Notez que dans la visualisation ci-dessus, le modèle à six segments ne peut être sélectionné avec une valeur de lambda. Il est donc impossible sélectionner ce modèle en cliquant sur le graphique.
<!-- comment -->
...
<!-- comment -->
Il y a un intervalle par signal ; nous créé un `animint` dans la section précédente pour le signal indiqué en noir.
<!-- comment -->
...
<!-- comment -->
On voit clairement qu’elle coupe chaque intervalle cible nois et maximise la marge (lignes pointillées verticales rouges).
<!-- comment -->
...
Vous savez comment visualiser les sept parties du jeu de données `intreg`. Le reste du chapitre est consacré à des exercices.
...
<!-- comment -->
- Dans le `animint` créé dans l’exercice précédent, ajoutez un troisième graphique incluant la fonction de sélection de modèle pour le signal sélectionné.
<!-- comment -->
...
<!-- comment -->
Un clic sur le graphique de sélection du modèle doit permettre de sélectionner le nombre de segments, en entraîner la mise à jour du modèle affiché et des erreurs d’étiquette sur le graphique pour le signal sélectionné.
<!-- comment -->
Ajoutez également une indication visuelle du modèle sélectionné sur le graphique de régression de la marge maximale.
<!-- comment -->
Le résultat devrait ressembler à [ceci](https://rcdata.nau.edu/genomic-ml/animint-gallery/2016-01-28-Max-margin-interval-regression-for-supervised-segmentation-model-selection/index.html).
<!-- comment -->
...
<!-- comment -->
Le résultat devrait ressembler à [ceci](https://rcdata.nau.edu/genomic-ml/animint-gallery/2016-11-10-Max-margin-supervised-penalty-learning-for-peak-detection-in-ChIP-seq-data/index.html).
