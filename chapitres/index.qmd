...
**Pour citer cet ouvrage :** Toby Hocking (2026). *Visualisation interactive de données dans R avec animint2*. Université de Sherbrooke, Département d’informatique. fabriqueREL. Licence CC BY-SA. <br /> <br />
...
Cette section est copié avec modifications de l’introduction du livre sur R, *Méthodes quantitatives en sciences sociales : un grand bol d’R* [@Apparicio2024].
...
-   Attribuer la paternité de l’auteur dans vos versions dérivées, ainsi qu’une mention concernant les grandes modifications apportées.
...
*  Modifier un plusieurs chapitres en effectuant les mises à jour directement dans les chapitres.
...
Il existe actuellement de nombreux ressources, surtout en anglais, pour l’apprentissage de la visualisation statique (non-interactive) avec la grammaire de graphiques.
En français il y a :
...
Il y a certaines ressources sur R en français qui propose un chapitre sur la visualisation interactive :
...
Ceci est un tradiction du livre en anglais [@Hocking-2026-animint-manual-en].
...
- **La table des matières doit être impérativement extensible !** Si vous êtes intéressé(e)s à ajouter un nouveau chapitre ou une partie du livre, nous vous invitons vivement à communiquer avec nous (avec « Signaler un problème ou
...
Le [chapitre 14](ch14) présente une visualisation avec de nombreux sélecteurs, à l’aide de `clickSelects` et `showSelected` nommés, dans une visualisation interactive appliqué à un modèle pour la détection de pics.
...
<!-- comment -->
Cette visualisation des données porte le nom de courbe de Keeling, du nom de Charles David Keeling, le premier scientifique à effectuer des relevés continus et systématiques des taux CO₂ atmosphérique [@keeling1960concentration].
<!-- comment -->
