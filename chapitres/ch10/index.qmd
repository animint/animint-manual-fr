...
<!-- comment -->
Voici un graphique de l’erreur de validation à 10 divisions et un graphique des prédictions du classificateur 7 plus proches voisins.
<!-- comment -->
...
Notre but dans ce chapitre est de reproduire une version interactive de la figure 13.4 tirée de [Elements of Statistical Learning](http://statweb.stanford.edu/~tibs/ElemStatLearn/) [@Hastie2009].
<!-- comment -->
Cette figure se compose de deux graphiques :

<!-- paragraph -->

![Esquisse pour K-Plus-Proches-Voisins](ch10-viz-interactive.png)

<!-- paragraph -->

À gauche : courbes d’erreur de mauvaise classification, en fonction du nombre de voisins.
...
<!-- comment -->
Pour `validation.fold=0`, on traite l’ensemble des 200 observations comme un ensemble d’entraînement, qui sera utilisé pour visualiser les frontières de décision apprises avec K Plus Proches Voisins.
...
Enfin, nous redéfinissons le graphique, en utilisant le nombre voisins (`neighbors`) comme variable `showSelected` dans plusieurs geoms.
...
Enfin, nous combinons les deux graphiques dans une visualisation des données unique avec le nombre de voisins (`neighbors`) comme variable de sélection.
...
Notez que le nombre de voisins (`neighbors`) sont utilisés comme variable de temps, de sorte que l’animation montre les prédictions des différents modèles.
...
<!-- comment -->
Nous avons commencé par une visualisation statique qui n’affichait que les prédictions des 7 Plus Proches Voisins.
<!-- comment -->
Ensuite, nous avons créé une visualisation interactive qui permettait de sélectionner K, le nombre de voisins.
<!-- comment -->
Nous avons procédé à une autre visualisation, en ajoutant une facette permettant de sélectionner le nombre de divsions dans la validation croisée.
...
<!-- comment -->
La difficulté principale est que la probabilité est une variable numérique, mais que `animint2` impose que chaque échelle soit exclusivement continue ou discrète (pas les deux).
<!-- comment -->
