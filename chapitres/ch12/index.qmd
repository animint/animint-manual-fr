...
L’objectif de ce chapitre est de créer une visualisation des données interactive pour expliquer le concept de [séparateur à vaste marge](https://fr.wikipedia.org/wiki/Machine_%C3%A0_vecteurs_de_support) (SVM), un modèle d'apprentissage automatique pour la classification binaire.
...
<!-- comment -->
- Dans la dernière section, nous réalisons une visualisation interactive pour montrer comment la limite de décision du SVM à noyau polynomial varie en fonction des deux hyperparamètres (coût et degré).
...
<!-- comment -->
Bien entendu, une telle visualisation serait impossible avec des données réelles (seules les étiquettes sont connues dans les données réelles, pas les scores).
...
Notez que nous avons utilisé deux graphiques multipanneaux avec l’option [addColumn then facet](/ch99#addColumn-then-facet) plutôt que quatre graphiques distincts.
<!-- comment -->
...
Pour chaque itération, nous utilisons le code du section précédente, avec la méthode [Liste de tableau de données](/ch99#list-of-data-tables).
...
* `slack` est la quantité d’erreur donnée par les variables ressorts.
...
Ci-dessus nous voyons l’espace au carré, utilisé pour l’apprentissage avec le noyau polynome d’ordre 2.
