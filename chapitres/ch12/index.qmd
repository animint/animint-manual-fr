...
Pour chaque itération, nous utilisons le code du section précédent, avec la méthode [Liste de tableau de données](/ch99#list-of-data-tables).
...
Ci-dessus nous voyons un tableau avec une ligne pour chaque coût.
Les colonnes donnent les propriétés de chaque modèle :

* `slack` est la quantité d’erreur donnée par les variables ressorts.
* `norm` est la norme du vecteur du poids.
...
Ensuite, nous faisons un graphique pour l’espace des variables carrées.
...
Ci-dessus nous voyons l’espace carrée, utilisée pour l’apprentissage avec le noyau polynome ordre 2.
Enfin, le code ci-dessous combine les graphiques dans une visualisation interactive.

```{r ch12-viz-linear-svm}
...
- Utilisez [la mise en page d'un tableau HTML](../ch06/ch06-other#html-table-layout) pour [`viz.linear.svm`](#linear-svm) de manière à ce que les deux graphiques de l'espace des variables apparaissent l'un à côté de l'autre et que le graphique « Choisir la régularisation » apparaisse au-dessus ou en-dessous.
<!-- comment -->
