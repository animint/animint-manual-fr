...
<!-- comment -->
* De créer des ggplots multicouches
<!-- comment -->
* De créer des ggplots multipanneaux
...
<!-- comment -->
ce qui a conduit en 2018, à la création du package `animint2` qui copie/fork les parties pertinentes du package `ggplot2`.
<!-- comment -->
Désormais, `animint2` peut être utilisé sans que `ggplot2` soit installé.
<!-- comment -->
Il est en fait conseillé d'utiliser `animint2` sans attacher `ggplot2` via `library`.
<!-- comment -->
On peut cependant utiliser `animint2` avec des packages qui importent/chargent `ggplot2`.
<!-- comment -->
À titre d'exemple, voir [Chapitre 16](Ch16-change-point.html) qui utilise le package `penaltyLearning` (lequel importe `ggplot2`).
...
Une fois  `animint2` installé, vous pouvez charger et attacher toutes ses fonctions exportées via :
...
<!-- comment -->
Nous utilisons comme exemple un ensemble de données de la Banque mondiale. Commençons par charger et examiner un sous-ensemble de ces données.
...
Le jeu de données `BanqueMondiale` rassemble des indicateurs tels que le taux de fécondité et l'espérance de vie de chaque pays entre 1960 et 2010.
<!-- comment -->
Le code ci-dessus imprime les premières et les dernières lignes, ainsi que les dimensions (lignes `r nrow(BanqueMondiale)` et colonnes `r ncol(BanqueMondiale)`) du tableau de données.
...
Le code ci-dessus imprime les données pour 1975, avec les colonnes appropriées et une ligne pour chaque pays
<!-- comment -->
...
<!-- comment -->
Le ggplot a été sauvegardé en tant qu'objet `scatter` qui, lorsqu'imprimé sur la ligne de commande R, affiche le graphique sur un périphérique graphique.
<!-- comment -->
...
<!-- comment -->
La fonction `animint()` crée une liste de classe `"animint"`, puis R exécute la fonction `print.animint()` grâce à au [système d'objets S3](http://adv-r.had.co.nz/OO-essentials.html#s3).
<!-- comment -->
...
<!-- comment -->
Le compilateur est la fonction `animint2dir()` qui compile la liste animint `nuage.vis` en un dossier de fichiers de données (CSV) et de codes (HTML, JavaScript) qui peuvent être affichés dans un navigateur web.
<!-- comment -->
Le compilateur est invoké par la fonction `print.animint`.
...
Notez que nous sauvegardons la valeur de retour de la fonction `animint()` dans l'objet `vis.deux.couches` (imprimé lui aussi en raison des parenthèses).
<!-- comment -->
...
* le `geom_point` montre l'espérance de vie, le taux de fécondité et la région de tous les pays en 1975.
<!-- comment -->
...
<!-- comment -->
Plus particulièrement que les données de la plupart des pays se sont déplacées vers la droite et vers le bas, ce qui signifie une espérance de vie plus élevée et un taux de fécondité plus faible.
<!-- comment -->
...
<!-- comment -->
Pour simplifier, nous réduisons d'abord l'échantillonnage des données à tous les cinq ans (certains explorateurs web comme Chromium ne sont pas en mesure d'afficher plus de 10 000 points de données en une seule fois),
...
Un ggplot peut avoir des facettes (panneaux), qui sont des sous-graphiques, chacun qui présent un différent sous-ensemble de données.
<!-- comment -->
...
<!-- comment -->
Notez également que nous avons écrit `facet=x.var`, alors nous devons donc ajouter une variable `x.var` à chacun des trois ensembles de données.
<!-- comment -->
...
<!-- comment -->
Nous appelons cette méthode [Les facettes économes en espace](Ch99-appendix.html#space-saving-facets) qui est généralement utile dans tout ggplot avec des facettes.
...
for(show.année in c(1975, 1985, 1995)){
  show.point.list[[paste(show.année)]] <- data.frame(
    show.année, subset(BanqueMondiale, année==show.année))
...
<!-- comment -->
* Pourquoi on utiliserait plusieurs couches dans un même graphique ?
<!-- comment -->
...
Ensuite, dans [Chapitre 3](Ch03-showSelected.html) nous expliquons le mot-clé `showSelected`, qui nous permettra de préciser l'intéractivité ainsi que l'animation.
