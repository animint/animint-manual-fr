...
<!-- comment -->
Vous pouvez utiliser `scale_color_manual` avec une palette de couleurs séquentielle. Pour plus de détails, consulter `RColorBrewer::display.brewer.all(type="seq")` et lire [l’annexe](Ch99-appendix.html#manual-color-legends).
...
<!-- comment -->
Nous appelons cette méthode [Ajouter une variable ensuite des facettes](Ch99-appendix.html#addColumn-then-facet). Elle est particulièrement utile pour créer une visualisation de données à plusieurs panneaux avec des axes alignés.
<!-- comment -->
...
<!-- comment -->
Il s’agit d’un exemple de la méthode [Liste de tableau de données](Ch99-appendix.html#list-of-data-tables), utile pour la visualisation interactive :
...
<!-- comment -->
Dans le [chapitre suivant](Ch03-showSelected.html) nous verrons comment le nouveau mot-clé `showSelected` peut être utilisé pour réaliser une animation, et nous révélerons plus de détails sur cet ensemble de données.
...
Dans le [chapitre 3](Ch03-showSelected.html) nous expliquons le mot-clé `showSelected`, qui nous permettra de préciser l'interactivité ainsi que l’animation.
