...
* Essayez d’utiliser la variable qualitative `revenu` dans `aes(color)` ou `aes(fill)`pour obtenir une légende interactive.
* Essayez `population` et `log10(population)` sur `x` ou `y`, vous verrez que la transformation logarithmique fonctionne bien pour cette variable (`population` varie de plusieurs ordres de grandeur).
* Essayez `aes(size=log10(population))`; vous obtiendrez une légende quantitative, qui ne sera pas interactive.
* Pour avoir une légende interactive pour `population`, il faudra discrétiser la variable. Créez une nouvelle variable discrète `log10pop` avec définition `paste(round(log10(population)))`. Si vous utilisez `aes(size=log10pop)` vous verrez une légende qualitative interactive.
