...
`aes(key)` a été présenté au [chapitre 3](Ch03-showSelected.html#duration-key); il sert à désigner une variable à utiliser pour obtenir des transitions fluides et interprétables.
...
Notez qu'une infobulle de forme « valeur variable » est définie par défaut pour chaque geom avec `aes(clickSelects)`.
<!-- comment -->
Par exemple, un geom avec `aes(clickSelects=année)` affiche l'infobulle par défaut « année 1984 » pour une observation correspondant à cette période.
<!-- comment -->
...
Le [chapitre 3](Ch03-showSelected.html#selecting-with-menus) introduit `showSelected` pour désigner un geom qui n'affiche que le sous-ensemble sélectionné de ses données.

<!-- paragraph -->

Le [chapitre 4](Ch04-clickSelects.html#clickSelects-keyword) introduit `clickSelects` pour désigner un geom sur lequel on peut cliquer pour modifier une variable de sélection.
...
[Chapitre 3](Ch03-showSelected.html#duration-key) a introduit l'option `duration` qui permet de spécifier la durée des transitions en douceur.

<!-- paragraph -->

[Chapitre 3](Ch03-showSelected.html#animation-time) a introduit l'option `time` qui permet de spécifier une variable de sélection qui est automatiquement mise à jour (animation).

<!-- paragraph -->

[Chapitre 4](Ch04-clickSelects.html#first-option) a introduit l'option `first` qui permet de spécifier la sélection lorsque la visualisation des données est générée pour la première fois.

<!-- paragraph -->

[Chapitre 4](Ch04-clickSelects.html#selector-types-option) a introduit l'option `selector.types` qui permet de spécifier plusieurs variables de sélection.
...
Le chapitre 5 a introduit la fonction [animint2pages](Ch05-sharing.html#pages) qui permet de publier un `animint2` sur GitHub Pages.
<!-- comment -->
Il faut que l'`animint2` définisse l'option `title` car ces métadonnées sont nécessaires à l'organisation de l'`animint2` dans une [galerie](Ch05-sharing.html#gallery) .
...
Le chapitre 5 a introduit la fonction [animint2pages](Ch05-sharing.html#pages) qui permet de publier un `animint2` sur GitHub Pages.
<!-- comment -->
Il faut que l'`animint2` définisse l'option `source` car ces métadonnées sont nécessaires à l'organisation de l'`animint2` dans une [galerie](Ch05-sharing.html#gallery) .
...
- les variables de sélection basées sur les données qui sont définies à l'aide de [namedclickSelects/showSelectedvariables](Ch06-other.html#data-driven-selectors);
<!-- comment -->
...
Le [chapitre 7](Ch07-limitations.html) explique les limites de l'implémentation actuelle de `animint2`.
