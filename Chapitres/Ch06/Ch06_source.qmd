...
Notez dans le code ci-dessus qu'il faut installer le package `maps` pour obtenir ces données.
Ensuite, on utilise le code ci-dessous pour tracer la carte.
On utilise `geom_polygon()` avec `aes(href)` qui donnera des hyperliens vers le page Wikipédia pour chaque état.
...
Cliquez sur le bouton « Show download status table » , et vous devriez voir des décomptes de chunks (fichiers TSV).
<!-- comment -->
...
<!-- comment -->
Si une autre valeur de `chunk_vars` était spécifiée, cela créerait un nombre différent de fichiers TSV, mais l'apparence de la visualisation de données devrait être la même.
...
<!-- comment -->
Lors du choix de la valeur de `chunk_vars`, vous devez tenir compte à la fois l'utilisation du disque et le temps de chargement.
<!-- comment -->
...
<!-- comment -->
À l'intérieur de `theme`, vous pouvez directement utiliser des nombres pour modifier la taille de la police, ou vous pouvez utiliser `rel()` pour définir la taille relative.
...
L'option `title` doit être une chaîne de caractères, et sera utilisée pour définir l'élément `\<title>` de la page web.
<!-- comment -->
Il n'est pas utile d'utiliser l'option `title` dans un document Rmd tel que cette page.
<!-- comment -->
...
<!-- comment -->
Il faut que l'`animint2` définisse l'option `title` car ces méta-données sont nécessaires à l'organisation de l'`animint2` dans une [galerie](Ch05-sharing.html#gallery) .
...
Le chapitre 5 a introduit la fonction [animint2pages](Ch05-sharing.html#pages) qui permet de publier un `animint2` sur GitHub Pages.
<!-- comment -->
Il faut que l'`animint2` définisse l'option `source` car ces méta-données sont nécessaires à l'organisation de l'`animint2` dans une [galerie](Ch05-sharing.html#gallery) .
...
L'option `video` doit être une chaîne de caractères : un lien vers une vidéo qui montre des interactions typiques avec l'`animint2`.
<!-- comment -->
...
<!-- comment -->
L'idée est que vous pouvez enregistrer une vidéo pour chacun de vos animints, puis inclure un lien vers cette vidéo à l'aide de ce mécanisme, afin que vos utilisateurs puissent comprendre plus facilement ce qui est affiché et quelles interactions sont possibles.
...
<!-- comment -->
Pour ceux qui souhaitent voir un exemple du fonctionnement de l'option `selectize`, veuillez consulter [PredictedPeaks dans le code source de `animint2`](https://github.com/tdhock/animint2/blob/master/tests/testthat/test-renderer2-PredictedPeaks.R) .
...
Ensuite, [Chapitre 7](Ch07-limitations.html) explique les limites de l'implémentation actuelle de `animint2`.
