...
Ce chapitre donne une liste complète des nouvelles fonctionnalités qu'animint introduit dans la grammaire graphique.
<!-- comment -->
Après avoir lu ce chapitre, vous comprendrez comment personnaliser vos graphiques animint via

<!-- paragraph -->

- le `href`, `tooltip`, `id` l'esthétique pour les caractéristiques propres à l'observation ;
<!-- comment -->
- les éléments nommés de `clickSelects` et `showSelected` pour spécifier plusieurs variables de sélection à la fois ;
<!-- comment -->
- le `chunk_vars` l'option spécifique au geom ;
<!-- comment -->
- l'option `color_off`, `fill_off`, et `alpha_off` paramètres geom permettant de spécifier le mode d'affichage de l'état de la sélection ;
<!-- comment -->
- le `help` et `title` les paramètres spécifiques au geom, qui peuvent être définis comme des chaînes de texte qui seront affichées dans la visite guidée.
<!-- comment -->
...
## Options spécifiques à l'observation (nouvelle esthétique) {#new-aesthetics}

<!-- paragraph -->

Cette section explique les nouvelles esthétiques reconnues par animint2.

<!-- paragraph -->

### Revue des esthétiques introduites précédemment {#review-aesthetics}

<!-- paragraph -->

Nous commençons par discuter de la nouvelle esthétique que nous avons déjà introduite dans les chapitres précédents.

<!-- paragraph -->

[Chapitre 3](Ch03-showSelected.html#duration-key) a également introduit `aes(key)` pour désigner une variable à utiliser pour des transitions fluides et interprétables.
...
Le code ci-dessous utilise animint pour dessiner une carte des États-Unis.
...
Essayez de cliquer sur un état dans l'image de données ci-dessus.
<!-- comment -->
...
Les infobulles sont de petites fenêtres d'information textuelle qui apparaissent lorsque vous passez le curseur sur un élément de l'écran.
<!-- comment -->
Dans animint, vous pouvez utiliser `aes(tooltip)` pour désigner le message spécifique à l'observation qui apparaît.
<!-- comment -->
...
Notez qu'une infobulle de la forme "valeur variable" est spécifiée par défaut pour chaque geom avec `aes(clickSelects)`.
<!-- comment -->
Par exemple, un geom avec `aes(clickSelects=year)` affiche l'info-bulle par défaut "année 1984" pour une observation dont l'année est 1984.
<!-- comment -->
...
Puisque tout ce qui est tracé par animint est affiché sous la forme d'un [SVG](http://www.w3schools.com/html/html5_svg.asp) dans une page Web, vous voudrez peut-être spécifier un élément [attribut HTML id](http://www.w3schools.com/tags/att_global_id.asp) à l'aide de l'attribut `aes(id)` comme ci-dessous.
...
Notez comment `gsub` est utilisé pour convertir les espaces en caractères de soulignement, puisqu'un identifiant bien défini ne doit pas inclure d'espaces.
<!-- comment -->
...
<!-- comment -->
Les développeurs d'animint2 utilisent cette fonctionnalité [pour tester le code de l'afficheur JavaScript de l'animint2](https://github.com/tdhock/animint/wiki/Testing) .

<!-- paragraph -->

### Noms de sélecteurs pilotés par les données en utilisant les noms clickSelects et showSelected {#data-driven-selectors}
...
<!-- comment -->
Par exemple, `showSelected=c("year", "country")` signifie créer deux variables de sélection (nommées `year` et `country` ).
<!-- comment -->
Cependant, cette méthode devient peu pratique si vous avez de nombreux sélecteurs dans vos données viz.
<!-- comment -->
Pour illustrer notre propos, nous considérons l'exemple théorique suivant (le code de cette section n'est pas directement exécutable).
<!-- comment -->
Supposons que vous souhaitiez utiliser 20 noms de variables de sélection différents, `selector1value`...
<!-- comment -->
`selector20value`.
<!-- comment -->
La façon habituelle de définir une image de données est la suivante
...
Cette méthode est mauvaise car elle est lente à construire `viz` et la viz compilée prend potentiellement beaucoup d'espace disque puisqu'il y a au moins un fichier TSV créé pour chaque `geom_point`.
<!-- comment -->
La méthode préférable est d'utiliser un vecteur de caractères nommés pour `clickSelects`.
<!-- comment -->
Les noms doivent être utilisés pour indiquer la colonne qui contient le nom de la variable du sélecteur.
<!-- comment -->
Par exemple, les noms doivent être utilisés pour indiquer la colonne qui contient le nom de la variable du sélecteur :
...
Le compilateur animint parcourt le data.frame `all.data` et crée des sélecteurs pour chacune des valeurs distinctes de `all.data$selector.name`.
<!-- comment -->
En cliquant sur l'un des points de données, le sélecteur correspondant est mis à jour avec la valeur indiquée dans le champ `all.data$selector.value`.

<!-- paragraph -->

De la même manière, vous pouvez utiliser un geom avec un élément nommé `showSelected` au lieu d'un tas de geom différents avec un nom `showSelected`.

<!-- paragraph -->

Cette caractéristique est utile non seulement pour éviter les répétitions dans la définition des données, mais aussi parce qu'elle est plus efficace en termes de calcul.
<!-- comment -->
Pour un exemple détaillé avec les temps et les mesures de l'espace disque, voir [Chapitre 14](Ch14-PeakSegJoint.html) .
...
Dans animint, il existe plusieurs options de personnalisation au niveau du geom : `chunk_vars` permet de spécifier la manière de diviser les ensembles de données pour le stockage sur disque et `\*_off` sont utilisés pour spécifier comment un geom clickSelects doit être affiché lorsqu'il n'est pas sélectionné.
<!-- comment -->
En outre, `help` et `title` peuvent être spécifiées, afin d'ajouter des informations à la visite guidée.

<!-- paragraph -->

### Le `chunk_vars` option de compilation spécifique au geom {#chunk-vars}

<!-- paragraph -->

Le `chunk_vars` définit les variables de sélection qui sont utilisées pour diviser l'ensemble de données en morceaux distincts (fichiers TSV) à télécharger.
<!-- comment -->
Un fichier TSV est créé pour chaque combinaison de valeurs de l'option `chunk_vars` variables.
<!-- comment -->
Plus de variables de sélection spécifiées en `chunk_vars` signifie qu'il faut diviser l'ensemble de données en plusieurs fichiers TSV, chacun d'une taille plus petite.

<!-- paragraph -->

Le `chunk_vars` doit être spécifiée en tant qu'argument de l'option `geom_\*` et sa valeur doit être un vecteur de caractères contenant des noms de variables de sélection.
<!-- comment -->
Lorsque `chunk_vars=character(0)` un vecteur de caractères de longueur zéro, toutes les données sont stockées dans un seul fichier TSV.
<!-- comment -->
Quand `chunk_vars` est défini à tous les `showSelected` un fichier TSV est créé pour chaque combinaison de valeurs de ces variables.

<!-- paragraph -->

En général, le compilateur animint choisit une valeur par défaut raisonnable pour les variables `chunk_vars` mais il se peut que vous souhaitiez spécifier `chunk_vars` si les données se chargent lentement ou prennent trop de place sur le disque.
<!-- comment -->
Si l'affichage des données se charge lentement, vous devriez ajouter des variables de sélection à l'affichage des données. `chunk_vars` afin de réduire la taille du premier fichier TSV à télécharger.
<!-- comment -->
Si l'image de données occupe trop d'espace sur le disque, vous devez supprimer les variables de sélection de l'image de données. `chunk_vars` afin de réduire le nombre de fichiers TSV.
<!-- comment -->
Un grand nombre de petits fichiers TSV peuvent occuper plus d'espace disque qu'un seul fichier TSV, car certains systèmes de fichiers stockent une quantité constante de métadonnées pour chaque fichier.

<!-- paragraph -->

Pour illustrer l'utilisation de `chunk_vars`, considérez la visualisation suivante de l'élément `breakpoints` de l'ensemble de données.

<!-- paragraph -->

![Données sur les points d'arrêt](Ch06-viz-breakpoints.png)

<!-- paragraph -->

L'esquisse ci-dessus se compose de deux graphiques.
<!-- comment -->
Nous commençons par créer le graphique des courbes d'erreur à gauche.
...
Le graphique non interactif ci-dessus comporte 80 facettes, une pour chaque combinaison des deux `showSelected` variables, `bases.per.probe` et `segments`.
<!-- comment -->
Nous présentons ci-dessous une version interactive dans laquelle une seule de ces facettes sera affichée.
