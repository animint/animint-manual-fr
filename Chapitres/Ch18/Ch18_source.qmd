...
<!-- comment -->
The latent/true function used for classification is the following,
...
The plot above shows each row in the data set as a point, with the two features on the two axes, and the two labels in two different colors.
<!-- comment -->
The lower right part of the feature space tends to have positive labels, and the left and top areas have negative labels.
<!-- comment -->
This is the pattern that the neural network will learn.
<!-- comment -->
To properly train a neural network, we need to split the data into two sets:

<!-- paragraph -->

- subtrain: used to compute gradients, which are used to update weight parameters, and predicted values.
<!-- comment -->
With enough iterations/epochs of the gradient descent learning algorithm, and a powerful enough neural network model (large enough number of hidden units/layers), it should be possible to get perfect prediction on the subtrain set.
<!-- comment -->
- validation: used to avoid overfitting.
<!-- comment -->
By computing the prediction error on the validation set, and choosing the number of gradient descent iterations/epochs which minimizes the validation error, we can ensure the learned model has good generalization properties (provides good predictions on not only the subtrain set, but also new data points like in the validation set).
...
The code above is used to randomly assign half of the data into each of the subtrain and validation sets.
<!-- comment -->
Below, we plot the two sets in separate facets,
...
To visualize the optimal/Bayes decision boundary, we need to evaluate the function on a 2d grid of points that spans the feature space.
<!-- comment -->
To create such a grid, we first create a list which contains the two 1d grids for each feature,
...
The plot above shows that even using the best possible function, there are still some prediction errors (points on a background of different color).
<!-- comment -->
Another way to visualize that best classification function is via the decision boundary, which can be computed using the code below,
...
<!-- comment -->
Auto-grad is the idea that the neural network model structure should be defined once, and that structure should be used to derive both the forward (prediction) and backward (gradient) propagation computations.
<!-- comment -->
...
<!-- comment -->
The forward/value and gradient computations use matrix multiplication.
<!-- comment -->
...
<!-- comment -->
To use the gradient features we need a loss function, which in the case of binary classification is the logistic (cross-entropy) loss,
...
The code above defines the logistic loss and gradient, assuming the label is either -1 or 1, and the prediction is a real number (not necessarily between 0 and 1, maybe negative).
<!-- comment -->
The code below creates nodes for the labels and loss,
...
Now that we have computed the loss, we can compute the gradient of the loss with respect to the weights, which is used to perform the updates during learning.
<!-- comment -->
...
<!-- comment -->
Below we first verify that it has not yet been computed, then we compute it:
...
<!-- comment -->
The gradient is the direction of steepest ascent, meaning the direction weights could be modified to maximize the loss.
<!-- comment -->
Because we want to minimize the loss, the learning algorithm performs updates in the negative gradient direction, of steepest descent.
...
In gradient descent for this linear model, we update the weight vector in this direction.
<!-- comment -->
Each update to the weight vector is referred to as an iteration or step.
<!-- comment -->
A small step in this direction is guaranteed to decrease the loss, but too small of a step will not make much progress toward minimizing the loss.
<!-- comment -->
It is unknown how far in this direction is best, so we typically need to search over a grid of step sizes (aka learning rates).
<!-- comment -->
Or we can perform a line search, which means making the following plot of loss as a function of step size, then choosing the step size with minimal loss.
...
Defining a linear model in the previous section was relatively simple, because there is only one weight matrix parameter (actually, a weight vector, with the same number of elements as the number of columns/features, plus one for an intercept).
<!-- comment -->
In contrast, a neural network has more than one weight matrix parameter to learn.
<!-- comment -->
We initialize these weights as nodes in the code below,
...
The output above shows that there is a single layer with 40 hidden units in the neural network, meaning there are two weight matrices to learn.
<!-- comment -->
Each of these weight matrices is used to predict the units in a given layer, from the units in the previous layer.
<!-- comment -->
In order to learn a prediction function which is a non-linear function of the features, each layer except for the last must have a non-linear activation function, applied element-wise to the units after matrix multiplication.
<!-- comment -->
...
The code below combines all of the pieces above into a gradient descent learning algorithm.
<!-- comment -->
The hyper-parameters are the constant step size, and the maximum number of iterations.
...
From the plot above we can see that as the number of iterations increases, the predictions get more accurate.
<!-- comment -->
Finally, we conclude with an interactive plot where you can click the loss plot to select an iteration of gradient descent, for which the corresponding decision boundary is shown on the predictions plot.
...
## Chapter summary and exercises {#ch18-exercises}
