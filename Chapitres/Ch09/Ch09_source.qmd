...
Tracer avec `geom_point` permet de voir facilement la différence entre les zéros et les valeurs manquantes.
...
<!-- comment -->
- Sur la carte, dessiner un cercle pour chaque localisation, dont la taille varie en fonction du comptage `count` des accidents dans le mois `month` sélectionné.
<!-- comment -->
...
<!-- comment -->
- Supprimer le `MonthSummary` et ajouter une visualisation similaire dans un troisième panneau de l'écran dans le graphique `MonthFacet`.

<!-- paragraph -->

Ensuite, le [Chapitre 10](Ch10-nearest-neighbors.html) explique comment visualiser le modèle d'apprentissage automatique des K-voisins les plus proches.
