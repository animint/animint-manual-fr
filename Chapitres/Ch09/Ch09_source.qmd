...
Pour chaque accident il y a des données sur la date, l'heure, la localisation et le nombre de morts et de blessés légers/graves.
<!-- comment -->
Certaines valeurs sont en français (par exemple : position Voie de circulation, localisation En intersection, etc).

<!-- paragraph -->

Nous calculons l'intervalle de temps des accidents ci-dessous.
...
Ci-dessous, nous calculons également la plage (ou l'intervalle) de mois pour le tableau de données du compteur de vélos.
...
Tracer les données avec `geom_point` permet de voir facilement la différence entre les zéros et les valeurs manquantes.

<!-- paragraph -->

Nous allons compiler un résumé de tous les accidents survenus dans le mois au cours de cet intervalle, c'est pourquoi nous commençons par créer un tableau de données pour chaque mois ci-dessous.
<!-- comment -->
(assurez-vous de définir l'environnement linguistique sur C pour les noms de mois en anglais)
...
Notez que nous avons créé une colonne `month` qui est un facteur ordonné par `month.levs`.
...
<!-- comment -->
Ci-dessous, nous calculons le nombre de X par mois.
...
On remarque que certains mois comportent des journées sans observations.
...
L'emplacement des compteurs ci-dessus sera tracé ci-dessous.
<!-- comment -->
