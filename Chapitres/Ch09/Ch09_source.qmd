...
library(data.table)
data(montreal.bikes, package="animint2") #only present if installed from github
...
ggplot()+
  ...
  geom_bar(aes(
    ...
    stat="identity",
    data=accidents.tall)+
  scale_fill_manual(values=gravité.colors, breaks=names(gravité.colors))+
...
(MonthSeries <- ggplot()+
  ...
  geom_tallrect(aes(
    xmin=mois01-15*un.jour, xmax=mois01+15*un.jour),
    clickSelects="mois.français",    
    data=mois_dt,
    alpha=1/2)+
  geom_line(aes(
    mois2POSIXct(mois.str), passages_mean, group=lieu,
    color=lieu),
    showSelected="lieu",
    clickSelects="lieu",
...
(MonthText <- MonthSeries+
  geom_point(aes(
    mois2POSIXct(mois.str), passages_mean, color=lieu,
    tooltip=paste(
    ...
    data=COMPTEURS(passages.par.mois))+
  geom_text(aes(
    mois2POSIXct(mois.str), passages_mean+300,
...
(MonthFacet <- MonthText+
  ...
  scale_fill_manual(values=gravité.colors, breaks=names(gravité.colors))+
  geom_bar(aes(
    mois2POSIXct(mois.str), personnes,
    fill=gravité),
...
```{r}
(weekend.dt <- add_jour_mois(days.dt[jds %in% c("sam", "dim")])[])
```
...
(jour.text <- passages_dt[, .SD[which.max(passages)], by=.(lieu, mois.français)])
```
