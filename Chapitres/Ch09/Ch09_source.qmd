...
-   We begin with some static data visualizations. <!-- comment -->
-   We create an interactive visualization of accident frequency over time. <!-- comment -->
...
<!-- comment --> Therefore to access this data set, you will need to install animint2 from GitHub:
...
<!-- comment --> Some of the values are in French (e.g. position Voie de circulation, location En intersection, etc).
...
<!-- comment --> (and make sure to set the locale to C for English month names)
...
<!-- comment --> Below we classify the severity of each accident according to the worst outcome among the people affected.
...
<!-- comment --> Below we compute counts per month.
...
<!-- comment --> The plot on the left shows that the number of accidents grows with the number of cyclists.
<!-- comment --> The plot on the right shows the frequency of accidents over time.
...
<!-- comment --> Each dot represents one person who got in an accident.
...
<!-- comment --> Note that we use `showSelected=month` and `clickSelects=location`.
...
<!-- comment --> Note that we use `geom_tallrect` with `clickSelects` to select the month.
...
-   Change location to a multiple selection variable. <!-- comment -->
-   Add a plot for the map to the data viz. <!-- comment -->
-   On the map, draw a circle for each location, with size that changes based on the `count` of the accidents in the currently selected `month`. <!-- comment -->
-   On the `MonthSummary` plot, add a background rectangle that can be used to select the `month`. <!-- comment -->
