...
ConvertRmd_comments /n Traduction de [English](https://github.com/tdhock/animint-book/) [Ch03-showSelected](https://raw.githubusercontent.com/tdhock/animint-book/master/Ch03-showSelected.Rmd)
...
En fait, nous avons déjà utilisé le mot-clé showSelected, qui a été créé automatiquement par les légendes interactives que nous avons créées dans les deux chapitres précédents.
A titre d'exemple, considérons le dessin ci-dessous de la légende interactive [visualisation de données de la courbe de Keeling du chapitre 1](Ch01-motivation.html#large-data) .
...
Le dessin ci-dessus comprend `showSelected=month` pour le `geom\_point` ce qui signifie qu'il doit afficher le sous-ensemble de données pour les mois sélectionnés.
En revanche, étant donné que l'élément `geom\_line` n'inclut pas `showSelected` il affiche toujours l'ensemble des données (quels que soient les mois sélectionnés).
...
Au lieu de cela, nous avons spécifié `aes(color=region)` et animint a automatiquement assigné un mot-clé showSelected.
En général, animint attribue un mot-clé showSelected pour chaque variable utilisée dans une légende qualitative.

<!-- paragraphe suivant -->

Cependant, le mot-clé showSelected n'est pas limité à une utilisation avec des légendes qualitatives.
...
Par exemple, la viz de données de la courbe de Keeling a une variable de sélection (mois), de même que la viz de données de la Banque mondiale (région).
...
Dans la section suivante, nous allons explorer une visualisation des données de la Banque mondiale qui comporte des variables de sélection pour `region` et `year` .
...
Notez également que les données sont spécifiées pour toutes les années (mais une seule sera affichée à la fois en raison de l'utilisation de la fonction `showSelected=year` ).
...
Notez que puisque `color=region` a été spécifié, animint utilise aussi automatiquement `region` en tant que `showSelected` variable.
...
Chaque variable possède un menu au bas de l'image de données qui peut être utilisé pour modifier la sélection en cours.
Dans cette image de données, ces menus de sélection sont affichés par défaut.
Ils peuvent être masqués en cliquant sur le bouton "Cacher les menus de sélection", et réaffichés en cliquant sur le bouton "Afficher les menus de sélection".
...
Dans l'animation ci-dessus, vous pouvez modifier la valeur de l'élément `region` en utilisant soit la légende, soit le menu.
...
Vous devriez voir les points du nuage de points se mettre à jour immédiatement pour afficher le taux de fécondité et l'espérance de vie de tous les pays au cours de l'année que vous avez sélectionnée.

<!-- paragraphe suivant -->

Exercice multicouche : Ajoutez un autre geom à ce nuage de points interactif.
Comme dans [Chapitre 2](Ch02-ggplot2.html#multi-layer) vous pouvez utiliser un `geom\_text` pour afficher le nom de chaque pays (facile), ou un `geom\_text` pour afficher l'année sélectionnée (moyen), ou a `geom\_path` pour afficher les données des 5 années précédentes (difficile).
Conseil : veillez à spécifier `showSelected=year` pour tous les geoms.

<!-- paragraphe suivant -->

Exercice de tracé multiple : Ajoutez un gracé de série temporelle à la viz de données ci-dessus.
Comme dans [Chapitre 2](Ch02-ggplot2.html#multi-plot) vous pouvez utiliser un `geom\_line` pour afficher le taux de fécondité de chaque pays pour toutes les années.
Ajoutez a `geom\_vline` avec `showSelected=year` pour mettre en évidence l'année sélectionnée.
...
Vous avez peut-être remarqué qu'il y a des boutons au bas de chaque visualisation de données créée par animint.
...
Pour illustrer l'importance des durées de transition, essayez de modifier la durée de transition de la variable year en 2000.
...
Certaines transitions se traduisent par un déplacement minime des points vers des positions proches (par exemple 1979-1980).
...
Le `key` dans le ggplot ci-dessus n'a de sens que pour la visualisation interactive des données, il est donc ignoré lorsqu'on l'affiche avec les périphériques graphiques R habituels.
...
Les `duration` doit être une liste nommée.
...
Si vous cliquez sur "Show animation controls" dans l'image de données ci-dessus, vous verrez que la zone de texte pour la variable année est 2000, comme spécifié dans le code R.
Si vous changez la sélection de 1980 à 1981, vous devriez voir une transition correcte.

<!-- paragraphe suivant -->

En général, le `key` doit être spécifiée pour tous les geoms qui utilisent l'esthétique `showSelected` avec une variable qui apparaît dans l'élément `duration` dans l'option.
Dans cet exemple, nous avons utilisé la variable `duration` pour spécifier une transitions fluides pour l'option `year` pour une transition en douceur.
Puisque nous utilisons l'option `showSelected=year` dans l'option `geom\_point` nous avons également spécifié la variable `key` esthétique pour ce geom.
...
Conseil : faites une liste de ggplots qui a le format `time` mais pas l'option `duration` option.
