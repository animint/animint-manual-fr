...
En revanche, étant donné que l'élément `geom_line` n'inclut pas `showSelected` il affiche toujours l'ensemble des données (quels que soient les mois sélectionnés).
...
Puisque nous utilisons l'option `showSelected=year` dans l'option `geom_point` nous avons également spécifié la variable `key` esthétique pour ce geom.
