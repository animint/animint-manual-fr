...
A la place, nous avons spécifié `aes(color=region)` et animint a automatiquement attribué un mot-clé showSelected.
...
Par exemple, l'outil de données de la courbe de Keeling comporte une variable de sélection (le mois), tout comme l'outil de données de la Banque mondiale (la région).
...
Dans la section suivante, nous explorerons une visualisation des données de la Banque mondiale qui comporte des variables de sélection pour les éléments suivants `region`  et `year` .
...
En revanche, les techniques de manipulation indirecte telles que les menus sont généralement plus compliquées à comprendre.
...
Vous devriez voir les points du nuage de points se mettre à jour immédiatement pour montrer le taux de fécondité et l'espérance de vie de tous les pays pour l'année que vous avez sélectionnée.
...
Certaines transitions n'entraînent qu'un léger déplacement des points vers des positions proches (par ex.
...
D'autres transitions se traduisent par des points qui se déplacent beaucoup plus, vers des localisations très éloignées (ex.
...
La variable `key`  dans le ggplot ci-dessus n'a de sens que pour la visualisation interactive des données, il est donc ignoré lorsqu'il est affiché avec les périphériques graphiques habituels de R.
...
Dans cet exemple, nous avons utilisé la fonction `duration`  pour spécifier une transitions fluides pour l'option `year`  pour la variable
...
Conseil : dressez une liste de ggplots contenant les éléments suivants `time`  mais pas d'option `duration`  option.
