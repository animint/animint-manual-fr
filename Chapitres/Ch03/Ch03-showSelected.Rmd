...
-   Utiliser le mot-clé showSelected dans vos dessins de graphe pour spécifier des geoms. pour lesquels seul un sous-ensemble de données doit être tracé à tout moment.
-   Utilisez les menus de sélection dans animint pour modifier le sous-ensemble de données tracées.
-   Spécifiez des transitions fluides entre les sous-ensembles de données en utilisant la durée et de l'esthétique des touches.
...
Par exemple, considérons le dessin ci-dessous de l'ensemble de données [visualisation de données de la courbe de Keeling du chapitre 1](Ch01-motivation.html#large-data).
...
En revanche, étant donné que le`geom_line` n'inclut pas `showSelected` il affiche toujours l'ensemble des données (quels que soient les mois sélectionnés).
...
Vous pouvez utiliser les mots-clés showSelected pour toutes les variables de données que vous souhaitez, en spécifiant explicitement les noms des variables dans l'argument showSelected de la commande geom.

Chaque variable utilisée avec showSelected est traitée par animint comme une variable de type *variable de sélection*.
Par exemple, la viz de données de la courbe de Keeling comporte une variable de sélection (le mois), tout comme la viz de données de la Banque mondiale (la région).
...
Toutefois, une visualisation de données peut comporter un nombre quelconque de variables de sélection.
...
Notez également que les données sont spécifiées comme étant toutes les années (mais une seule sera affichée à la fois en raison de l'option `showSelected=year`).
...
Notez que puisque`color=region` a été spécifié, animint utilise aussi automatiquement le mot-clé `region` comme `showSelected` variable.
...
Notez que la visualisation de données ci-dessus comporte deux variables de sélection : la région et l'année.
Chaque variable dispose d'un menu en bas de l'image de données qui peut être utilisé pour modifier la sélection actuelle.
...
**Essayer** de changer la région sélectionnée dans la légende interactive et dans le menu de sélection.
...
**Exercice à plusieurs niveaux :** Ajoutez un autre geom à ce nuage de points interactif.
...
**Exercice sur plusieurs graphes :** Ajoutez un graphe de série temporelle à la vue de données ci-dessus.
...
Ajoutez a`geom_vline` avec `showSelected=year` pour mettre en évidence l'année sélectionnée.
...
Ensuite, modifiez la valeur sélectionnée de la variable year.
...
D'autres transitions entraînent un déplacement beaucoup plus important des points, vers des localisations plus éloignées (par ex. 1980-1981).
...
Dans le code R ci-dessous, nous calculons un tableau de contingences des points de données qui peuvent être tracés pour chacune de ces trois années.
...
En effet, le tableau de contingence ci-dessus montre que le taux de fécondité et l'espérance de vie sont manquants pour le Kosovo durant la période 1979-1980.
...
Le `key` dans le ggplot ci-dessus n'a de sens que pour la visualisation interactive des données, elle est donc ignorée lorsqu'elle est affichée avec les périphériques graphiques habituels de R.
...
Pour spécifier une durée de transition par défaut pour la variable `year` nous utilisons l'option `duration` dans l'image de données ci-dessous.
...
En règle générale, l'animation `key` doit être spécifiée pour tous les geoms qui utilisent l'esthétique `showSelected` avec une variable qui apparaît dans l'élément `duration` option.
Dans cet exemple, nous avons utilisé la variable`duration` pour spécifier une transitions fluides pour l'option `year`pour la variable Puisque nous utilisons `showSelected=year` dans la `geom_point` nous avons également spécifié l'option `key` esthétique pour ce geom.
...
-   Réaliser une version améliorée de `viz.aligned` à partir de la version précédente précédent. Au lieu de fixer l'année à 1975, utilisez `showSelected=year` pour que l'utilisateur puisse choisir une année. Ajouter des geom qui indiquent l'année sélectionnée : un `geom_text` sur le nuage de points, et a `geom_vline` sur la série chronologique.
-   Traduisez l'un des [exemples de packages d'animation](https://yihui.name/animation/examples/) à une animint. Conseil : dans le code du package d'animation, il y a toujours une boucle for sur la variable time. Au lieu d'appeler un fonction de tracé à l'intérieur de la boucle for, utilisez la fonction [liste de tableaux de données.](Ch99-appendix.html#list-of-data-tables) pour stocker les données qui doivent être tracées. Ces données sont ensuite utilisées avec avec `showSelected` pour créer des ggplots, et les afficher en utilisant animint.
