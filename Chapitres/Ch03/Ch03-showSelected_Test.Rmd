...
Multi-layer exercise:  Add another geom to this interactive  scatterplot. As in  [Chapter 2](Ch02-ggplot2.html#multi-layer) , you can  use a  `geom_text`  to show the name of each country (easy), or a  `geom_text`  to show the selected year (medium), or a  `geom_path`  to  show the previous 5 years of data (hard). Hint: make sure to specify  `showSelected=year`  for all geoms.

<!-- paragraphe suivant -->

Multi-plot exercise:  Add a time series plot to the data viz  above. As in  [Chapter 2](Ch02-ggplot2.html#multi-plot) , you can use a  `geom_line`  to show the fertility rate for each country over all  years. Add a  `geom_vline`  with  `showSelected=year`  to highlight the  currently selected year.
