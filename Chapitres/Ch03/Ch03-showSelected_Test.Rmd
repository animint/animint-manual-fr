...
In this section, we will explain how the showSelected keyword can be  used in plot sketches.
<!-- comment -->
The showSelected keyword specifies a variable  to use for subsetting the data before plotting.
<!-- comment -->
Each geom in a data  visualization has its own data set, and its own definition of  showSelected variables.
<!-- comment -->
That means different geoms can specify  different data sets and showSelected keywords to show different data  subsets.

<!-- comment -->

In fact, we have already used the showSelected keyword, which was  automatically created by the interactive legends that we created in  the previous two chapters.
<!-- comment -->
For example, consider the sketch below of  the  [Keeling Curve data visualization from Chapter 1](Ch01-motivation.html#large-data) .
...
The sketch above includes  `showSelected=month`  for the  `geom_point` ,  meaning that it should show the subset of data for the selected  months.
<!-- comment -->
In contrast, since the  `geom_line`  does not include  `showSelected`  keywords, it always shows the entire data set  (regardless of the selected months).
...
<!-- comment -->
Instead, we specified  `aes(color=region)` , and  animint automatically assigned a showSelected keyword.
<!-- comment -->
In general, animint will assign  a showSelected keyword for each variable that is used in a  categorical legend.
...
Each variable that is used with showSelected is treated by  animint as a  selection variable .
<!-- comment -->
For example, the Keeling Curve data  viz has one selection variable (month), and so does the WorldBank data  viz (region).
<!-- comment -->
For each selection variable, animint keeps track of the  currently selected values.
<!-- comment -->
When the selection changes, animint updates  the subset of data that is shown.

<!-- comment -->

Each of the data visualizations sketched above has only one selection  variable.
<!-- comment -->
However, a data visualization can have any number of  selection variables.
<!-- comment -->
In the next section, we will explore a  visualization of the World Bank data that has selection variables for  `region`  and  `year` .
...
Note that there are two  `showSelected`  variables,  `region`  and  `year` .
<!-- comment -->
Also note that the data is specified as all years (but only  one will be shown at a time due to  `showSelected=year` ).
<!-- comment -->
...
Note that the ggplot above contains the  `showSelected`  argument, one  of the two main features introduced in animint2.
<!-- comment -->
The  `showSelected`  keyword is ignored when rendering the plot using the usual R graphics  devices, which produce a scatterplot with one point for every country  and year.
<!-- comment -->
Note that since  `color=region`  was specified, animint also  automatically uses  `region`  as a  `showSelected`  variable.
...
Note that the data viz above has two selection variables: region and  year.
<!-- comment -->
Each variable has a menu at the bottom of the data viz that can  be used to change the current selection.
<!-- comment -->
In this data viz, these  selection menus are shown by default.
<!-- comment -->
They can be hidden by clicking  the "Hide selection menus" button, and shown again by clicking the  "Show selection menus" button.

<!-- comment -->

Discrete legend variables such as region default to multiple  selection, so several values are selected and shown at once.
<!-- comment -->
Try  changing the selected region in the interactive legend and the  selection menu.
<!-- comment -->
When you change the selection using either method,  both the interactive legend and the selection menu should update to  reflect the current selection.

<!-- comment -->

We use the terms "direct manipulation" and "indirect manipulation" to  describe these different ways of changing the selection.
<!-- comment -->
Direct  manipulation typically involves clicking on the objects that you want  to change, and is usually easier to understand.
<!-- comment -->
In contrast, indirect  manipulation techniques such as menus are typically more complicated  to understand.
<!-- comment -->
In the animint above, you can change the value of the  `region`  variable using either the legend or the menu.
<!-- comment -->
Using the  legend is a more direct manipulation technique, since the legend is  drawn closer to the plotted data points that will be updated.

<!-- comment -->

Other selection variables such as year default to single selection, so  only one value is selected and shown at any time.
<!-- comment -->
Try  changing the  selected value of the year variable using the selection menu.
<!-- comment -->
You  should see the points in the scatterplot immediately update to show  the fertility rate and life expectancy of all the countries in the  year that you selected.

<!-- comment -->

Multi-layer exercise:  Add another geom to this interactive  scatterplot.
<!-- comment -->
As in  [Chapter 2](Ch02-ggplot2.html#multi-layer) , you can  use a  `geom_text`  to show the name of each country (easy), or a  `geom_text`  to show the selected year (medium), or a  `geom_path`  to  show the previous 5 years of data (hard).
<!-- comment -->
Hint: make sure to specify  `showSelected=year`  for all geoms.

<!-- comment -->

Multi-plot exercise:  Add a time series plot to the data viz  above.
<!-- comment -->
As in  [Chapter 2](Ch02-ggplot2.html#multi-plot) , you can use a  `geom_line`  to show the fertility rate for each country over all  years.
<!-- comment -->
Add a  `geom_vline`  with  `showSelected=year`  to highlight the  currently selected year.
...
Some transitions result in points moving only a little bit, to nearby  positions (e.g. 1979-1980).
<!-- comment -->
Other transitions result in points moving  a lot more, to far away locations (e.g. 1980-1981).
<!-- comment -->
Why is that?
...
<!-- comment -->
Then use those data along  with  `showSelected`  to create ggplots, and render them  using animint.
