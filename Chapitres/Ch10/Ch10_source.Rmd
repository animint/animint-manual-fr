...
# Chapitre 10, exemple des K-voisins les plus proches
...
<!-- comment -->
Il y a un graphique de l'erreur de validation croisée 10 fois, et un graphique des prédictions du classificateur 7-Plus Proches Voisins.
<!-- comment -->
...
<!-- comment -->
- `px1`, la grille de points pour la première caractéristique d'entrée (69 valeurs numériques entre -2,6 et 4,2).
<!-- comment -->
Ces points seront utilisés pour calculer la limite de décision de Bayes à l'aide de la fonction contourLines.
<!-- comment -->
- `px2` la grille de points pour la deuxième caractéristique d'entrée (99 valeurs numériques entre -2 et 2,9).
<!-- comment -->
...
Nous construisons ci-dessous des tableaux de contingence pour l'erreur de Bayes (dont nous savons qu'elle est de 0,21 pour les données de l'exemple de mélange) et l'erreur de formation/essai.
...
getBoundaryDF <- function(prob.vec){
  ...
  for(path.i in seq_along(several.paths)){
    contour.list[[path.i]]
<!-- comment -->
<- with(several.paths[[path.i]], data.table(
      path.i, V1=x, V2=y))
  }
...
![Viz KNN interactive](Ch10-viz-interactive.png)
...
On redéfinit le graphique des courbes d'erreur ci-dessous.
<!-- comment -->
Notez que
...
<!-- comment -->
- Nous ajoutons une légende pour montrer quels points de données d'entraînement sont mal classés.
<!-- comment -->
...
error.by.folds[["10"]]
<!-- comment -->
<- data.table(n.folds=10, validation.error)
...
Le code ci-dessous crée une visualisation des données interactive en utilisant le nouveau tracé de la courbe d'erreur.
...
<!-- comment -->
Ensuite, nous avons créé un re-design interactif qui permet de sélectionner K, le nombre de voisins.
<!-- comment -->
...
<!-- comment -->
Comment modifieriez-vous la visualisation pour afficher la probabilité prédite plutôt que la classe prédite à chaque point de la grille ?
<!-- comment -->
Le principal défi est que la probabilité est une variable numérique, mais ggplot2 impose que chaque échelle soit continue ou discrète (pas les deux).
<!-- comment -->
...
<!-- comment -->
Assurez-vous que la taille tracée des ensembles de validation et de formation change en fonction de la valeur sélectionnée de `n.folds`.
<!-- comment -->
