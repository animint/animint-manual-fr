...
<!-- comment -->
- Enfin, nous remanions la visualisation des données interactive avec des légendes simplifiées et des tallrects mobiles.
...
Le graphique ci-dessus montre le chemin d'accès complet du lasso, les pondérations optimales dans le problème de régression par moindres carrés régularisés L1, pour chaque paramètre de régularisation lambda.
<!-- comment -->
...
<!-- comment -->
Plus précisément, comme le LARS ne calcule que les points de changement dans le chemin linéaire par morceaux, sa complexité en temps ne dépend que du nombre de points de changement (et non du nombre de points de la grille).

<!-- paragraph -->

## Visualisation interactive du chemin d'accès de régularisation {#interactive-path-viz}
...
<!-- comment -->
Premièrement, vous avez peut-être remarqué qu'il y a deux légendes de jeu différentes dans l'animint précédent (linetype=set dans le premier graphique de chemin d'accès, et color=set dans le deuxième graphique de résidus).
<!-- comment -->
...
<!-- comment -->
- Ajoutez un autre nuage de points qui montre les valeurs prédites en fonction de la réponse, avec une fonction `geom_abline` en arrière-plan pour indiquer une prédiction parfaite.
<!-- comment -->
...
Suivant, [Chapitre 12](Ch12-SVM.html) explique comment visualiser la machine à vecteur de support.
