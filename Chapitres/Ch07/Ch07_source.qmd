...
Les statistiques, `geom_*(stat=...)`, et les positions, `geom_*(position=...)`, sont calculées avec toutes les données (et non pour chaque valeur de `showSelected`).
<!-- comment -->
...
Dans le graphique ci-dessus, on voit que `position="stack"` fait que le groupe `bas` est aligné avec `comptage=0` dans chaque facette.
...
Dans le graphique ci-dessus, si vous cliquez sur le menu de sélection pour changer `facette=2`, vous voyez que le groupe `bas` n'est plus aligné avec `comptage=0`.
Une solution de contournement consiste à calculer les valeurs que vous voulez afficher, puis à utiliser `position=identity`, comme dans le code ci-dessous.
...
Dans le graphique ci-dessus, si vous cliquez sur le menu de sélection pour changer `facette=2`, vous voyez que le groupe `bas` reste aligné avec `cumsum=0`.
...
* Vous pouvez utiliser `geom_label_aligned()` qui marche avec `vjust`. Pour exemple, lire le [chapitre 8](/ch08).

<!-- paragraph -->

Il est possible d'implémenter une prise en charge de `vjust` pour `geom_text()`, mais nous n'avons pas encore eu le temps d'y travailler.
<!-- comment -->
