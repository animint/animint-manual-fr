...
Si vous voulez mettre à jour un animint qui a déjà été publié sur GitHub Pages, vous pouvez simplement lancer `animint2pages(nouvelle_version, "répertoire_existant")` qui mettra à jour la branche `gh-pages` du répertoire spécifié.
...
- `repos.txt` (liste des répertoires sur GitHub qui contiennent des animints, un propriétaire/répertoire par ligne) et
...
<!-- comment -->
- prenez une capture d'écran de cet `animint2`, et validez/poussez cette capture d'écran dans un fichier nommé `Capture.PNG` (sensible à la casse), dans la branche`gh-pages` de `votre_identifiant_github/répertoire_de_vis` ;
<!-- comment -->
