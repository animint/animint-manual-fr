...
<!-- comment -->
- publiés en utilisant [GitHub Pages](https://pages.github.com/) et organisés dans un groupe une galerie.

<!-- paragraph -->

## Compiler dans un dossier local {#animint2dir}
...
<!-- comment -->
Plutôt que de sauvegarder chaque animint dans un dossier temporaire séparé, vous pouvez spécifier un dossier de sortie via l'argument `out.dir` de la commande `animint`.
<!-- comment -->
...
Si le parent d'`out.dir` n'existe pas, c'est une erreur (vous pouvez utiliser `dir.create` pour créer le parent si nécessaire).
<!-- comment -->
...
<!-- comment -->
Pour regarder la visualisation, il suffit d'aller à `Ch05-partager-dix-points/index.html` dans un navigateur web (ce qui devrait être fait automatiquement / par défaut).
<!-- comment -->
...
animint2dir(
  vis, 
  out.dir="Ch05-partager-dix-points-again", 
  open.browser=FALSE)
```
...
[Netlify Drop](https://app.netlify.com/drop) est un service d'hébergement de sites web statiques. Pour y publier votre visualisation de données, il suffit de faire glisser un dossier sur cette page web (il peut s'agir d'un dossier issu de `animint2dir` ou de `rmarkdown::render` si votre animint est à l'intérieur de Rmd, comme décrit ci-dessus).
<!-- comment -->
...
<!-- comment -->
Pour la plupart des visualisations, cette limite ne devrait pas poser de problème. Si votre visualisation de données contient un fichier TSV de plus de 100 Mo, vous pouvez utiliser l'option [chunk_vars](https://rcdata.nau.edu/genomic-ml/animint2-manual/Ch06-other.html#geom-options) pour convertir ce grand fichier en plusieurs fichiers plus petits.
