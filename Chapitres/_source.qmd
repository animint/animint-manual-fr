...
L’ordre de lecture des chapitres suivants n’est pas important, puisqu’ils expliquent comment réaliser des visualisations de données pour un ensemble de données ou à un algorithme statistique particulier. Chaque chapitre est associé à un modèle ou un algorithme, comme indiqué ci-dessous :

<!-- paragraph -->

[Chapitre 8](ch08) : des données de la Banque mondiale (visualisation multipanneaux).

<!-- paragraph -->

[Chapitre 9](ch09) : des données provenant de cyclistes de Montréal.
<!-- paragraph -->

[Chapitre 10](ch10) : des données sur les plus proches voisins tirées du livre Elements of Statistical Learning (Hastie, Tibshirani, et Friedman, 2009) [@Hastie2009].

<!-- paragraph -->

[Chapitre 11](ch11) : le Lasso, un modèle d’apprentissage automatique pour la régression régularisée.

<!-- paragraph -->

[Chapitre 12](ch12) : le séparateur à vaste marge (aussi connu sous le nom de « machine à vecteurs de support » ou SVM), un modèle d’apprentissage automatique pour la classification binaire.

<!-- paragraph -->

[Chapitre 13](ch13) : le modèle de régression de Poisson.
<!-- paragraph -->

[Chapitre 14](ch14) : une visualisation avec de nombreux sélecteurs, à l’aide de `clickSelects` et `showSelected` nommés, dans une visualisation interactive appliquée à un modèle pour la détection de pics.

<!-- paragraph -->

[Chapitre 15](ch15) : l’algorithme de recherche de racines « root-finding » de Newton.

<!-- paragraph -->

[Chapitre 16](ch16) : un modèle de détection de ruptures dans les données séquentielles.

<!-- paragraph -->

[Chapitre 17](ch17) : l’algorithme d’agrégation « clustering », k-moyennes.

<!-- paragraph -->

[Chapitre 18](ch18) : l’algorithme de descente de gradient pour l’apprentissage d’un réseau de neurones.

<!-- paragraph -->

[Chapitre 19](ch19) : visualisations de probabilités critiques (valeurs p) d'un test statistique, pour évaluer la significativité des différences entre deux échantillons. Un trop grand nombre de données dans un nuage de points cause de l'occlusion, nous proposons donc une carte thermique reliée à un nuage comportant seulement les données sélectionnées. 

[Chapitre 20](ch20) : une comparaison de différents algorithmes d’apprentissage artificiel, calculée avec le package mlr3.
...
<!-- comment -->
D’après Larousse, « donnée » signifie « représentation conventionnelle d'une information en vue de son traitement informatique » [@larousse_donnée]. https://www.larousse.fr/dictionnaires/francais/donnée
La représentation conventionelle pour `animint2` est un tableau, avec une ligne pour chaque observation, et une colonne pour chaque variable.
<!-- comment -->
...
<!-- comment -->
Le terme "visualisation des données" fait référence à la fois à l'image elle-même (également connue sous le nom de graphique, figure, diagramme, graph, plot ou visualisation des données) et au processus de création de l'image.
...
<!-- comment -->
Selon `help(fisher.test)` dans R, la dame a correctement identifié trois des quatre tasses dans lesquelles du lait a été ajouté après le thé.
<!-- comment -->
...
<!-- comment -->
Pour de tels ensembles de "grandes données", les approches traditionnelles d'analyse des données consistent à résumer les données, puis à visualiser ce résumé.
<!-- comment -->
Cependant, le résumé graphique peut être trompeur, car il ne montre pas tous les détails des données originales.
<!-- comment -->
...
L'exemple ci-dessus fournit également un bon exemple de `clickSelects` et `showSelected`, les deux mots-clés introduits par `animint2` pour permettre l'interaction.
<!-- comment -->
Sans entrer dans les détails, le graphique ci-dessus utilise `clickSelects=mois` pour la légende interactive, ce qui signifie qu'un clic sur la légende devrait modifier les mois sélectionnés.
<!-- comment -->
En outre, nous avons utilisé `showSelected=mois` pour les points, ce qui signifie que nous ne devons tracer que l'ensemble de points qui correspond aux mois actuellement sélectionnés.
<!-- comment -->
...
Ensuite, dans le [chapitre 2](ch02), nous vous expliquerons comment la grammaire graphique peut être utilisée pour créer des visualisations des données.
