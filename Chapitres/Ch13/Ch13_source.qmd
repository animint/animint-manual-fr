...
L'objectif de ce chapitre est de créer une visualisation des données interactive qui explique [la régression de Poisson][(https://en.wikipedia.org/wiki/Poisson_regression)](https://fr.wikipedia.org/wiki/R%C3%A9gression_de_Poisson), un modèle d'apprentissage automatique pour la prédiction d'une sortie à valeur entière à partir d'entrées sous forme de vecteurs à valeur réelle.
<!-- comment -->
Il s'agit d'un modèle de "régression linéaire" puisqu'il apprend une fonction linéaire des entrées à la sortie.
<!-- comment -->
...
![Visualisation des probabilités dans la régression de Poisson](../Ch13/Ch13-viz-prob.png)
...
Pour ajouter un panneau à la fonction de distribution cumulative, nous allons modifier le ggplot en suivant l'esquisse ci-dessous.

<!-- paragraph -->

![Visualisation des probabilités cumulées dans la régression de Poisson](../Ch13/Ch13-viz-cum-prob.png)

<!-- paragraph -->

Lorsque nous spécifions les ensembles de données, nous utilisons l'idiome [addColumn then facet](../Ch99/Ch99-appendix.html#addColumn-then-facet) pour ajouter une variable `panel`.
...
Notez l'utilisation d'un `geom_hline` pour indiquer le seuil de la fonction de distribution cumulative, 0,99, servant à déterminer l'ensemble des points à tracer pour chaque distribution de Poisson.
<!-- comment -->
...
Ci-dessous, nous calculons la perte pour plusieurs valeurs d'étiquettes, en utilisant l'idiome [tableau de données](../Ch99/Ch99-appendix.html#list-of-data-tables).
...
Dans le [chapitre 14](../Ch14/Ch14-PeakSegJoint.html), nous vous expliquerons comment utiliser les `clickSelects/showSelected` explicites pour visualiser le modèle d'apprentissage automatique PeakSegJoint avec des variables de sélection basées sur les données.
