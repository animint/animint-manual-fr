...
Notez que nous avons également utilisé un `geom_hline` pour indiquer le seuil de la fonction de distribution cumulative, 0,99, qui a été utilisé pour déterminer l'ensemble des points à tracer pour chaque distribution de Poisson.
<!-- comment -->
...
Ensuite, dans le [chapitre 14](../Ch14/Ch14-PeakSegJoint.html), nous vous expliquerons comment utiliser les `clickSelects/showSelected` nommés pour visualiser le modèle d'apprentissage automatique PeakSegJoint avec des variables de sélection basées sur les données.
