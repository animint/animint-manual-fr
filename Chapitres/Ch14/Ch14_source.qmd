# Nommé clickSelects/showSelected
...
Ce chapitre explique comment utiliser [les variables nommées clickSelects/showSelected](../Ch06/Ch06-other.html#data-driven-selectors) pour créer des noms de sélecteurs basés sur des données.
<!-- comment -->
...
- Nous commençons par télécharger le client \<- serveur>. `PSJ` et à calculer les données à tracer.
<!-- comment -->
...
<!-- comment -->
- Nous expliquons ensuite la méthode préférée pour définir une animint avec de nombreux sélecteurs, en utilisant les noms clickSelects/showSelected.
<!-- comment -->
...
Les données de l'exemple proviennent de la base de données [package PeakSegJoint](https://github.com/tdhock/PeakSegJoint).
<!-- comment -->
...
## Calcul des données à tracer {#compute-data-to-plot}

<!-- paragraph -->

La section ci-dessous calcule quelques données courantes que nous utiliserons dans deux visualisations des données ci-dessous.
...
## Visualisation des données à l'aide de boucles "for". {#define-using-for-loops}

<!-- paragraph -->

Le code R ci-dessous construit une visualisation des données à l'aide de boucles for.
...
Notez la synchronisation du code ci-dessus.
<!-- comment -->
L'évaluation du code R qui définit cette visualisation des données prend beaucoup de temps, car il y a beaucoup de geoms.
<!-- comment -->
...
Notez que la compilation prend également beaucoup de temps, car il y a beaucoup de geoms.
<!-- comment -->
La visualisation des données peut être consultée à l'adresse suivante [Ch14-pour/index.html](../Ch14/Ch14-for/index.html).
<!-- comment -->
Dans la section suivante, nous allons créer la même visualisation des données, mais de manière plus efficace.

<!-- paragraph -->

## Visualisation des données à l'aide de clickSelects/showSelected {#define-using-named}

<!-- paragraph -->

Dans cette section, nous utilisons named clickSelects/showSelected pour créer une version plus efficace de la visualisation des données précédente.
<!-- comment -->
...
Il est clair qu'il faut beaucoup moins de temps pour évaluer le code R ci-dessus qui utilise le nom clickSelects/showSelected.
<!-- comment -->
...
L'animation produite ci-dessus peut être visualisée sur [Ch14-nommé/index.html](../Ch14/Ch14-named/index.html).
<!-- comment -->
Notez qu'il devrait être identique à l'autre visualisation des données ci-dessus.
<!-- comment -->
Les temps ci-dessus montrent que les fonctions clickSelects/showSelected nommées sont beaucoup plus rapides que les boucles for, à la fois dans les étapes de définition et de compilation.
...
Le tableau ci-dessus montre que la visualisation des données définie à l'aide de boucles for prend environ deux fois plus d'espace disque que la visualisation des données utilisant les fonctions clickSelects/showSelected.
...
<!-- comment -->
Il est clair que les fonctions clickSelects/showSelects nommées sont plus efficaces dans les deux cas et devraient être utilisées à la place des boucles for.
...
- Utilisez named clickSelects/showSelected pour créer une visualisation de certaines données de votre domaine d'expertise.

<!-- paragraph -->

Suivant, [Chapitre 15](../Ch15/Ch15-Newton.html) explique comment visualiser les algorithmes de recherche de racines.
