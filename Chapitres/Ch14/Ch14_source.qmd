...
Ce chapitre explique comment utiliser les [variables `clickSelects` et `showSelected` nommées](/ch06#data-driven-selectors) pour créer des noms de sélecteurs basés sur des données.
<!-- comment -->
...
- Nous commençons par joindre le jeu de données `PSJ` et compiler les données à tracer.
<!-- comment -->
- Nous montrons une méthode pour définir un `animint` avec de nombreux sélecteurs, en utilisant des boucles `for`.
<!-- comment -->
Cette méthode est techniquement correcte, mais inefficace sur le plan computationnel.
<!-- comment -->
