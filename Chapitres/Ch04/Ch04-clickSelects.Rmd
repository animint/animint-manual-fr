...
<!-- comment -->
Elle affiche également une étiquette de texte pour les États-Unis.
...
<!-- comment -->
Nous devons donc créer un nouvel ensemble de données appelé `years` avec une ligne pour chaque année des données `BanqueMondiale`.
...
Exercice avec multicouches : ajoutez un `geom_text` qui indique l'année en cours sur le nuage de points.
<!-- comment -->
...
<!-- comment -->
- Utilisez l'option `time` pour créer une version animée de `viz.facets`.
<!-- comment -->
- Utiliser `help`  et `title`  pour chaque geom dans `viz.facets` afin de créer une tour guidée plus informative sur ce qui est affiché dans chaque geom.
