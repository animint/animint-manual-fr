...
<!-- comment -->
Le mot-clé `clickSelects` précise quelle variable de séléction sera mise à jour lorsqu'on clique sur le geom correspondant.
<!-- comment -->
Chaque geom d'une visualisation possède son propre ensemble de données et sa propre définition du mot-clé `clickSelects`.
<!-- comment -->
...
<!-- comment -->
- recourir au mot-clé `clickSelects` dans vos esquisses de graphiques;
<!-- comment -->
- traduire vos esquisses de graphiques avec `clickSelects` en code R;
<!-- comment -->
- spécifier plusieurs variables de sélection en utilisant l'option `selector.types`.
...
![Esquisse avec clickSelects en légende](ch04-legend-clickSelects.png)

<!-- paragraph -->

Puisque la légende indique `clickSelects=région`, cliquer sur une entrée de cette légende met à jour la variable de sélection `région`.
<!-- comment -->
...
<!-- comment -->
Chaque geom possède sa propre définition de `clickSelects`, qui détermine la variable de sélection devant être mise à jour.
<!-- comment -->
...
Notez que le graphique ci-dessus n'est pas interactif, car il s'affiche à l'aide du périphérique graphique R traditionnel.
<!-- comment -->
En revanche, afficher le même ggplot en utilisant `animint2` produit un graphique interactif : 

* `showSelected="année"` limite l'affichage aux données de l'année sélectionnée.
* `duration=list(année=2000)` définit un délai de 2 sécondes pour les transitions fluides entre les années.
...
<!-- comment -->
Vous devriez également constater que le point corrspondant au pays sélectionné est plus foncé que les autres.
<!-- comment -->
Cette mise en évidence de la sélection en cours est générée automatiquement pour chaque geom avec `clickSelects`.
<!-- comment -->
...
<!-- comment -->
Ces valeurs peuvent être personnalisées; par exemple, dans le code ci-dessous, un contour noir est utilisé pour mettre en évidence la sélection en cours.
...
La visualisation ci-dessus affiche le nom du pays sélectionné dans le menu de sélection, mais il serait préférable de l'afficher sous forme d'une étiquette de texte sur le nuage de points.
<!-- comment -->
...
Après avoir cliqué sur un point dans le nuage ci-dessus, vous devriez voir apparaître une étiquette de texte avec le nom du pays.
<!-- comment -->
Essayez également de changer l'année à l'aide du menu de sélection; l'étiquette de texte devrait se déplacer dans une transition fluide en même temps que le point de données correspondant. 

<!-- paragraph -->

La visualisation ci-dessus contient plus d'un geom, chacun avec des caractéristiques interactives différentes.
<!-- comment -->
Essayez de cliquer sur le bouton « Start Tour » (démarrer la visite guidée) au bas de la visualisation.
Une fenêtre s’affichera pour expliquer les fonctions interactives disponibles pour le premier geom.
<!-- comment -->
En cliquant sur « Next », vous obtiendrez des informations sur le prochain geom. Enn cliquant sur « Done » ou sur l'arrière-plan gris, vous mettrez fin à la visite guidée.
<!-- comment -->
La visite guidée aidera les nouveaux utilisateurs à découvrir les fonctions interactives présentes dans chaque geom.
<!-- comment -->
Les informations affichées pendant la visite sont personnalisables, il suffit de définir les paramètres `help` et `title` de chaque geom (voir le[chapitre 6 ](/ch06/ch06-viz#custom-guided-tour) pour un exemple).
...
<!-- comment -->
Cela signifie qu'une étiquette de texte ne sera dessinée que pour les lignes de la base de données `BanqueMondiale` qui correspondent aux valeurs actuelles des deux variables de sélection.
<!-- comment -->
Étant donné que chaque combinaison de `pays` et `année` correspond à une ligne dans ces données, une seule étiquette sera affichée à la fois.

<!-- paragraph -->

Essayez de cliquer sur l'entrée de la légende qui correspond à la région du pays actuellement sélectionné (par exemple, si le Canada est sélectionné, cliquez sur l'entrée de la légende Amérique du Nord).
<!-- comment -->
Vous devriez voir le point disparaître, mais le texte restera affiché.
...
<!-- comment -->
Dans la visualisation ci-dessus, nous pouvons modifier la valeur de la variable de sélection `pays` en cliquant sur un point de données (manipulation directe) ou en utilisant le menu de sélection (manipulation indirecte).
<!-- comment -->
Les deux techniques répondent à des besoins différents :

<!-- paragraph -->

- La manipulation directe (cliquer sur les points de données) est utile pour trouver le nom des pays ayant des valeurs extrêmes de taux de fertilité et d'espérance de vie.
<!-- comment -->
Par exemple, pour l'année 1960, en cliquant sur le point en bas à gauche du graphique, on découvre le nom du pays, le Gabon.
<!-- comment -->
- La manipulation indirecte à l'aide des menus est utile pour voir la position d'un pays.
<!-- comment -->
Par exemple, il serait difficile de localiser la France en cliquant sur différents points, mais plus simple de la trouver en tapant son nom dans le menu de sélection.

<!-- paragraph -->

Notez que lors du premier affichage de la visualisation ci-dessus, le pays sélectionné est Andorre, et l'année, 1960.
<!-- comment -->
...
Exercice :  Recréez la visualisation des données ci-dessus de manière à ce qu'un clic sur une étiquette de texte supprime le pays en question de l'ensemble de sélection.
<!-- comment -->
...
<!-- comment -->
Vous devriez voir les points de données et les étiquettes de texte se déplacer dans une transition graduelle vers une position correspondant à l'année nouvellement sélectionnée.
...
<!-- comment -->
Nous avons expliqué comment l'option `first` peut être utilisée pour préciser les valeurs à utiliser lorsque le `animint2` est affiché pour la première fois.
<!-- comment -->
...
- Jusqu'à présent, nous avons vu trois façons différentes de modifier les variables de sélection : (1) utiliser des légendes interactives, (2) utiliser des menus de sélection et (3) cliquer sur les données à l'aide de `clickSelects`.
<!-- comment -->
Classez ces trois techniques de manipulation, de la plus directe, à la moins directe.
<!-- comment -->
Quelle technique privilégier et selon quelles circonstances ?
<!-- comment -->
...
Dans le [Chapitre 5](Ch05-sharing.html) nous expliquons les différentes méthodes pour publier et partager des animints sur le web.
