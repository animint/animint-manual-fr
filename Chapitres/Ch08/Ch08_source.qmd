...
- Nous commençons par charger le jeu de données de la Banque mondiale et définir quelques fonctions d'aide pour créer un ggplot multipanneaux avec plusieurs geoms.
<!-- comment -->
...
Le tableau ci-dessus montre que la population du Koweït a diminué entre 1991 et1995, ce qui concorde avec la guerre du Golfe survenue dans cette période.
<!-- comment -->
...
Nous définissons ensuite la fonction d'aide suivante utilisée pour ajouter des colonnes aux ensembles de données afin d'affecter des geoms aux facettes.
...
<!-- comment -->
Il s'agit un exemple de l'idiome [add Column then facet](Ch99-appendix.html#addColumn-then-facet) .
<!-- comment -->
...
Nous définissons `panel.margin` à 0, une bonne idée pour [économiser de l'espace dans un ggplot avec des facettes](Ch99-appendix.html#space-saving-facets).
<!-- comment -->
Dans un des exemples tiré de, dans un exemple de l'idiome [ add Column then facet](Ch99-appendix.html#addColumn-then-facet),  nous utilisons `scales="free"` et nous masquons les étiquettes des axes.
<!-- comment -->
...
Nous ajoutons ci dessous des `widerects` pour sélectionner les années, et des chemins d'accès ("path") pour afficher le taux de fertilité.
