...
En particulier, nous supposons que l'étiquette `y` est négative (-1) si `x1.sq + noise < threshold` et positive (1) dans le cas contraire.
<!-- comment -->
...
L'`animint2` ci-dessous utilise `clickSelects` pour montrer quels points de l'espace d'entrée et de l'espace quadrillé correspondent.
<!-- comment -->
...
<!-- comment -->
Notez que nous avons également masqué la légende des couleurs dans le premier graphique, puisqu'il suffit d'avoir une seule légende des couleurs.
...
Nous commençons par ajuster un Séparateur à Vaste Marge (SVM) linéaire aux données d'entraînement dans l'espace des fonctionnalités au carré, et par visualiser les vraies étiquettes `y` avec les étiquettes prédites `pred.y`.
...
L'objectif ci-dessous est de créer un `animint2` qui montre comment la limite de décision, la marge et le ressort ("slack") évoluent en fonction du paramètre de coût.
...
<!-- comment -->
Nous avons utilisé `animint()` et l'interactivité pour montrer comment la limite de décision du SVM change en fonction des hyper-paramètres du modèle.
