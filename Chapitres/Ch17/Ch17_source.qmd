...
<!-- comment -->
Then K random data points are selected as the initial cluster centers,
...
Above we displayed the two data sets (cluster centers and data) using two instances of `geom_point()`.
<!-- comment -->
Below we compute the distance between each data point and each cluster center,
...
There are `r nrow(seg.dt)` segments overplotted above, so interactivity would be useful to emphasize the segments connected to a particular data point.
<!-- comment -->
To do that we create a`data.i` selection variable,
...
So the visualizations above show the steps of k-means: (1) updating cluster assignment based on closest center, then (2) updating center based on data assigned to that cluster.
<!-- comment -->
To visualize several iterations of the above two steps, we can use a for loop,
...
## Chapter summary and exercises {#ch17-exercises}
...
<!-- comment -->
For example in the viz above, iteration=16 was the last one that resulted in a decrease in error (iterations 17-20 resulted in no decrease).
<!-- comment -->
Modify the code so that it stops iterating if there is no decrease in error.
<!-- comment -->
- Current viz has only one animation frame (`showSelected` subset) per iteration (the mean shown is before it is updated).
<!-- comment -->
Add another animation frame that shows the mean after the update.
<!-- comment -->
...
<!-- comment -->
Compute the Adjusted Rand Index using `pdfCluster::adj.rand.index(species, cluster)` for each different K and seed.
<!-- comment -->
Add an overview plot that shows the ARI value of each model, and allows selecting the number of clusters.
<!-- comment -->
- Make a similar visualization using another data set such as `data("penguins", package="palmerpenguins")`.

<!-- paragraph -->

Next, [Chapter 18](/ch18) explains how to visualize the gradient descent learning algorithm for neural network learning.
