...
Dans ce chapitre, nous allons explorer plusieurs visualisations des données relatives à la [les valeurs P](https://en.wikipedia.org/wiki/P-value):
...
<!-- comment -->
- nous expliquons comment convertir un nuage de points surdéveloppé en une carte thermique liée et un nuage de points zoomé.
<!-- comment -->
...
<!-- comment -->
Par exemple, dans le cas d'un médicament pour la perte de poids, nous aimerions savoir si le poids du groupe traité est significativement inférieur à celui du groupe témoin.
<!-- comment -->
Supposons que 10 personnes soient assignées au groupe de contrôle et 15 au groupe de traitement.
<!-- comment -->
Nous pourrions ensuite calculer une valeur P à l'aide d'un test T unilatéral non apparié pour évaluer la signification statistique (non apparié parce que chaque personne du groupe de traitement n'a pas de membre correspondant dans le groupe de contrôle).
<!-- comment -->
- Dans une expérience d'apprentissage automatique, le réseau neuronal est-il plus précis que le modèle linéaire ?
<!-- comment -->
Par exemple, considérons un ensemble de données de référence de classification d'images, et une évaluation utilisant la validation croisée 5 fois.
<!-- comment -->
Nous aurions cinq mesures de la précision du test pour chacun des deux algorithmes d'apprentissage (réseau neuronal et modèle linéaire).
<!-- comment -->
Nous pourrions ensuite calculer une valeur P à l'aide d'un test T unilatéral apparié pour évaluer la signification statistique (apparié parce que la précision du test de pli 1 pour le modèle linéaire a une précision de test de pli 1 correspondante pour le réseau neuronal, etc.)

<!-- paragraph -->

Après avoir calculé les mesures (pondération de chaque personne ou précision des tests de chaque algorithme d'apprentissage automatique), nous pouvons les utiliser comme données d'entrée pour `t.test()` qui calculera une valeur P (plus elle est petite, plus la différence est significative).
<!-- comment -->
Pour comprendre la valeur P, nous devons d'abord adopter la méthode *hypothèse nulle* nous supposons qu'il n'y a pas de différence entre les conditions.
<!-- comment -->
...
- `trial` est le nombre de fois que nous répétons l'expérience (pour chaque décalage et écart-type).
...
Le résultat ci-dessus montre plusieurs centaines de milliers de lignes, une pour chaque normale aléatoire simulée. `value` (dont la moyenne dépend de `true_offset` et de `condition`).

<!-- paragraph -->

## Test T et graphique du volcan {#t-test-and-volcano-plot}

<!-- paragraph -->

Dans cette section, nous calculons les résultats des tests T et les visualisons à l'aide d'un tracé en volcan, qui est un graphique des valeurs P en logarithme négatif par rapport aux tailles d'effet estimées.
<!-- comment -->
La visualisation des résultats du test est appelée graphique en forme de volcan parce que la distribution typique des points ressemble à un volcan en éruption de l'origine vers le haut, vers la gauche et vers la droite.
...
- `true_tile` est une chaîne de texte permettant de sélectionner une combinaison d'un décalage et d'un écart-type.
<!-- comment -->
...
<!-- comment -->
Le code ci-dessous calcule un test T pour chaque répétition de la simulation :
...
La sortie ci-dessus comporte une ligne pour chaque test T et des colonnes pour la différence moyenne (`mean_zero`) et la `p.value`.
<!-- comment -->
Comme il y a dix échantillons par essai, il y a dix fois moins de lignes que dans le tableau de données simulé original.
<!-- comment -->
Chaque essai implique un test T de 5 échantillons de contrôle/zéro, contre 5 échantillons de traitement/décalage.
<!-- comment -->
Ensuite, nous ajoutons des colonnes pour le graphique du volcan :

<!-- paragraph -->

- `diff_means` est la différence entre les moyennes des deux conditions, parfois appelée "taille de l'effet".
- `neg.log10.p` est la valeur P transformée en logarithme négatif (plus elle est grande, plus elle est significative).
...
Ensuite, nous dessinons le tracé du volcan :
...
Le graphique statique ci-dessus montre un point par résultat de test T.
<!-- comment -->
...
<!-- comment -->
Il comprend également `fill=true_offset` et `color=sd` afin que nous puissions voir comment les paramètres de simulation affectent le graphique du volcan :

<!-- paragraph -->

- Plus grand `sd` apparaissent en bas (plus il y a de variance, plus il est difficile de détecter une différence).
<!-- comment -->
- Les couleurs plus foncées apparaissent près des côtés gauche/droit (des décalages réels plus importants tendent à se traduire par des différences de moyennes calculées plus grandes).

<!-- paragraph -->

Enfin, le graphique ci-dessus est surreprésenté, ce qui signifie que nous ne pouvons pas voir tous les détails, car il y a trop de points de données tracés les uns sur les autres.
...
Dans cette section, nous montrons comment le graphique de volcan précédent peut être révisé pour montrer plus de détails, en utilisant une carte thermique liée à un nuage de points zoomé.
<!-- comment -->
Pour commencer, nous définissons l'élément `round_rel()` ci-dessous, qui est utilisée pour ajouter `round_*` et `rel_*` que nous utiliserons pour définir les tuiles de la carte thermique.

<!-- paragraph -->

- `round_*` les colonnes sont arrondies à la taille de bac la plus proche (entière par défaut), et sont utilisées pour définir la tuile de la carte thermique. `x` et `y` de la carte thermique.
<!-- comment -->
- `rel_*` sont utilisées pour `x` et `y` dans l'affichage zoomé, et sont des unités relatives à la valeur correspondante de `round_*` correspondantes.
...
Ensuite, nous définissons `volcano_tile` une combinaison de chaînes de texte de `round_*` qui sera utilisée pour la sélection.
...
La sortie ci-dessus montre une ligne par tuile de la carte thermique, avec le paramètre `tests` indique le nombre de points apparaissant dans la zone correspondante du graphique du volcan.
<!-- comment -->
...
Le résultat ci-dessus est une carte thermique, les régions plus foncées indiquant les zones du graphique du volcan qui ont plus de résultats de tests.
<!-- comment -->
Le code ci-dessous combine la carte thermique du volcan avec un nuage de points agrandi, en utilisant la fonction `volcano_tile` pour les relier.
...
<!-- comment -->
Le test T est utilisé pour déterminer s'il existe une différence de moyennes entre les deux conditions.

<!-- paragraph -->

- Quand `true_offset=0` il n'y a pas de différence entre les deux conditions, de sorte que le test T devrait avoir une petite taille d'effet et une grande valeur P.
<!-- comment -->
- Lorsque `true_offset=2` il y a une plus grande différence entre les deux conditions, de sorte que le test T devrait avoir une taille d'effet plus importante et une petite valeur P.

<!-- paragraph -->

Le code ci-dessous crée un nouveau `significant` qui indique si le test rejette l'hypothèse nulle au seuil traditionnel de 5 %.
...
<!-- comment -->
Comme il y avait trop de tests T à montrer sur le graphique du volcan, nous avons plutôt utilisé une carte thermique liée à un diagramme de dispersion zoomé.
<!-- comment -->
...
- En `viz.volcano` En cliquant sur l'un des boutons de la partie inférieure de l'écran, on obtient une vue d'ensemble de la situation. `volcanoTiles` on ne voit qu'une partie de l'espace occupé en `volcanoZoom`.
<!-- comment -->
Pour corriger ce problème et occuper tout l'espace, revenez à la définition de `round_neg.log10.p` et utiliser l'élément `offset=0.5` dans `round_rel()` qui décale les valeurs relatives d'un certain décalage.
<!-- comment -->
- En `viz.volcano`, ajouter `aes(tooltip)` à `volcanoTiles` pour indiquer le nombre de points dans chaque tuile de la carte thermique.
<!-- comment -->
- Notez que deux `geom_tile()` ont été utilisés dans `viz.volcano` et deux `geom_rect()` ont été utilisés en `viz.parameters`.
<!-- comment -->
